...
class PairingStore:
    ...
    ``<NASTECH_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<NASTECH_HOME>/pairing/``
    directory (backward-compat for the ``nastech pairing`` CLI).
    """
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            self._dir = get_nastech_home() / "profiles" / profile / "pairing"
        else:
