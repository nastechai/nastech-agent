...
def _nastech_version() -> str:
    """Return the nastech-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``nastech_cli.__version__`` (covers editable /
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/chat — one synchronastechai agent turn."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
