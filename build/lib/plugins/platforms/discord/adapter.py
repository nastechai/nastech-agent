...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = env_float(
            "NASTECH_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "NASTECH_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Nastech: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("NASTECH_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["NASTECH_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("NASTECH_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["NASTECH_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'nastech-agent[messaging]'",
