...
# Scaffolding synthetic-turn flags that must never be persisted.
# The agent loop injects synthetic assistant/user messages during recovery,
# verification, and pre-verify nudges.  These messages are popped before a
...
class AIAgent:
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # Nastechai serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "nastechai":
            return False
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-nastechai-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-nastechai-credits-* headers "
                    "(miss — producer off / non-Nastechai path / >TTL stale)"
                )
            return
    ...
    def _try_refresh_nastechai_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "nastechai":
            return False
        ...
        try:
            ...
            logger.debug("Nastechai credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Nastechai requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="nastechai_credential_refresh"):
            return False
