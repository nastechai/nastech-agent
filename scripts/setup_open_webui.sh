...
# Bootstrap Open WebUI against Nastech Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.nastech/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-nastech.sh
...
#   OPEN_WEBUI_NAME='Johnny Nastech'
...
#   NASTECH_API_PORT=8642
#   NASTECH_API_HOST=127.0.0.1
#   NASTECH_API_MODEL_NAME='Nastech Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Nastech Agent WebUI}"
...
NASTECH_ENV_FILE="${NASTECH_ENV_FILE:-$HOME/.nastech/.env}"
NASTECH_API_PORT="${NASTECH_API_PORT:-8642}"
NASTECH_API_HOST="${NASTECH_API_HOST:-127.0.0.1}"
NASTECH_API_CONNECT_HOST="${NASTECH_API_CONNECT_HOST:-127.0.0.1}"
NASTECH_API_MODEL_NAME="${NASTECH_API_MODEL_NAME:-Nastech Agent}"
NASTECH_API_BASE_URL="http://${NASTECH_API_CONNECT_HOST}:${NASTECH_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-nastech.sh"
LOG_DIR="$HOME/.nastech/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$NASTECH_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$NASTECH_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.nastech.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.nastech</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.nastech"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.nastech"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-nastech.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Nastech Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-nastech.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.nastech/logs/openwebui.log
StandardError=append:%h/.nastech/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-nastech.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$NASTECH_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Nastech API server is configured...'
  upsert_env API_SERVER_ENABLED true "$NASTECH_ENV_FILE"
  upsert_env API_SERVER_HOST "$NASTECH_API_HOST" "$NASTECH_ENV_FILE"
  upsert_env API_SERVER_PORT "$NASTECH_API_PORT" "$NASTECH_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$NASTECH_API_MODEL_NAME" "$NASTECH_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$NASTECH_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Nastech gateway so API server settings take effect...'
  nastech gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${NASTECH_API_CONNECT_HOST}:${NASTECH_API_PORT}/health" >/dev/null; then
    log 'Nastech API server did not answer on the first check. Trying to start gateway in the background...'
    nohup nastech gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${NASTECH_API_CONNECT_HOST}:${NASTECH_API_PORT}/health" >/dev/null
  ...
  log "Nastech API endpoint: ${NASTECH_API_BASE_URL}"
