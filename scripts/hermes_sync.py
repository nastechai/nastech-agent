...
def sync_commits(
    ...
    for sha, subj in commits:
        ...
        # Commit
        branded_msg = brand_commit_message(subj)
        try:
...
def create_pr(branch: str, base: str, synced: list[str], skipped: list[str], full: bool) -> str:
    count = len(synced)
    title = f"chore(sync): upstream parity — {count} commit(s)"
    if full:
        title = "chore(sync): full upstream parity sync"

    lines = [
        f"## Upstream parity sync — {count} commit(s)",
        "",
        f"Nastechai branding applied to all touched files.",
        "",
    ]
    if full:
        lines += [
            "### Mode: full tree sync",
            "All files branded and written as a single squash commit.",
            "",
        ]
    if synced:
    ...
    lines += [
        "---",
        "*Generated by `scripts/hermes_sync.py` · [upstream source](https://github.com/NousResearch/hermes-agent)*",
    ]
...
def main() -> None:
    ...
    print(f"  upstream HEAD: {hermes_head[:12]}")
    ...
    if args.full:
        ...
        if not args.dry_run:
            ...
            git("commit", "--allow-empty",
                "-m", f"chore(sync): full upstream parity ({touched} files)")
    ...
    if not args.dry_run:
        ...
        git("commit", "--allow-empty",
            "-m", f"chore(sync): advance sync state to {hermes_head[:12]}")
