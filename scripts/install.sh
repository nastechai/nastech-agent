...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   Installs optional deps + runs nastech setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# string clears that floor; anything below it is replaced with the Nastech-
...
check_node() {
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Nastech-managed Node $NODE_VERSION LTS..."
...
setup_path() {
    ...
    if [ ! -x "$NASTECH_BIN" ]; then
        log_warn "nastech entry point not found at $NASTECH_BIN"
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Nastech for pip install"
    ...
    NASTECH_CMD="$(command -v nastech 2>/dev/null || echo "")"
    if [ -n "$NASTECH_CMD" ]; then
        log_info "Running nastech setup..."
        "$NASTECH_CMD" setup
    else
        log_warn "nastech command not found on PATH"
        log_info "Try: python -m nastech_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Nastech-managed Node to PATH, so
    ...
    # trips Gatekeeper's tamper check ("Nastech is damaged and can't be opened").
