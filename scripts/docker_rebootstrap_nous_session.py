...
Background
----------
A Nous bootstrap session (client_id ``nastech-cli-vps``) can take a terminal
...
``NASTECH_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a NASTECH_AUTH_JSON_REBOOTSTRAP
