#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead nastechai bootstrap session.

Background
----------
A nastechai bootstrap session (client_id ``nastech-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.nastechai.last_auth_error.relogin_required = true``. From then on every
...
- Surgical: replaces ONLY ``providers.nastechai`` in the existing auth.json, leaving
...
def _nastechai_entry_is_terminal(nastechai_state: Any) -> bool:
    """True iff the on-disk nastechai provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``nastech_cli.auth.get_nastechai_session_validity``:
    ...
    """
    if not isinstance(nastechai_state, dict):
        return False
    last_err = nastechai_state.get("last_auth_error")
    ...
    if nastechai_state.get("access_token") or nastechai_state.get("refresh_token"):
        return False
    return True
...
def _extract_nastechai_from_seed(seed_raw: str) -> Optional[dict]:
    ...
    if not isinstance(nastechai, dict) or not nastechai:
        return None
    if nastechai.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(nastechai.get("access_token"), str)
        and nastechai["access_token"].strip()
        and isinstance(nastechai.get("refresh_token"), str)
        and nastechai["refresh_token"].strip()
    ):
        return None
    return nastechai
...
def _seed_is_newer(local_nastechai: Any, seed_nastechai: dict) -> bool:
    ...
    """
    if not isinstance(local_nastechai, dict):
        return False
    local_obtained = _parse_timestamp(local_nastechai.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_nastechai.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    ...
    seed_nastechai = _extract_nastechai_from_seed(seed_raw)
    if seed_nastechai is None:
        return "bad_seed"
    ...
    local_nastechai = providers.get("nastechai")
    terminal = _nastechai_entry_is_terminal(local_nastechai)
    newer_seed = _seed_is_newer(local_nastechai, seed_nastechai)
    ...
    # Surgical replacement: swap ONLY providers.nastechai, preserve everything else.
    providers["nastechai"] = seed_nastechai
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] nastechai bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued nastechai bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
