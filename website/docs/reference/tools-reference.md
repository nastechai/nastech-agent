...
| `read_terminal` | Read what's currently shown in the in-app terminal pane of the NasTech desktop GUI (the embedded shell beside this chat). Desktop-app only. | — |
| `close_terminal` | Close the read-only terminal tab for a background process in the NasTech desktop GUI. Does NOT kill the process — only drops the tab/view; use process(action='kill') to stop it. Desktop-app only. | — |
| `open_preview` | Open a web URL, localhost dev-server URL, or file path in the preview pane beside the chat in the NasTech desktop app. Desktop-app only. | — |
| `read_preview` | Read what's currently shown in the preview pane of the NasTech desktop GUI — the in-app Browser's page text (URL + title + rendered text, pageable with `start`/`count`), or a file/artifact tab's identity. Desktop-app only. | — |
| `focus_pane` | Reveal and focus a pane in the NasTech desktop app (chat, files, terminal, review, sessions). Desktop-app only. | — |
