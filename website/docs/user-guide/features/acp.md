...
description: "Use Nastech Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains"
---

# ACP Editor Integration

Nastech Agent can run as an ACP server, letting ACP-compatible editors talk to Nastech over stdio and render:
...
ACP is a good fit when you want Nastech to behave like an editor-native coding agent instead of a standalone CLI or messaging bot.
...
Install Nastech normally, then add the ACP extra:
...
For Zed registry installs, Zed launches Nastech through the official ACP Registry entry. That entry uses a `uvx` distribution that runs:

```bash
uvx --from 'nastech-agent[acp]==<version>' nastech-acp
```
...
This is the standalone command. The Zed registry's terminal-auth flow (`nastech acp --setup`) also offers the browser bootstrap as a follow-up question after model selection, so most users never need to run `--setup-browser` directly.

What it does:

- Installs Node.js 22 LTS into `~/.nastech/node/` if missing
...
3. Search for **Nastech Agent**.
4. Install it and start a new Nastech external-agent thread.

Prerequisites:

- Configure Nastech provider credentials first with `nastech model`, or set them in `~/.nastech/.env` / `~/.nastech/config.yaml`.
- Install `uv` so the registry launcher can run `uvx --from 'nastech-agent[acp]==<version>' nastech-acp`.
...
```text
/path/to/nastech-agent/acp_registry
```

## Registry manifest

The source copy of Nastech' official ACP Registry metadata lives at:
...
The upstream registry PR copies those files into the top-level `nastech-agent/` directory in `agentclientprotocol/registry`.

The registry entry uses a `uvx` distribution that points directly at the `nastech-agent` PyPI release:
...
Provider resolution uses Nastech' normal runtime resolver, so ACP inherits the currently configured provider and credentials. Nastech also advertises a terminal auth method (`--setup`) for first-run registry clients; this opens Nastech' interactive model/provider setup.
...
- In Zed, open the ACP Registry with `zed: acp registry` and search for **Nastech Agent**.
- For manual/local development, verify the custom `agent_servers` command points to `nastech acp`.
...
or by editing `~/.nastech/.env`. Registry clients can also trigger Nastech' terminal auth flow, which runs the same interactive provider/model setup.

### Zed registry launcher cannot find uv

Install `uv` from the official uv installation docs, then retry the Nastech Agent thread from Zed.
