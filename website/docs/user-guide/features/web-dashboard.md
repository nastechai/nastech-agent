...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `nastech dashboard register` (NasTechai Portal) or install a
...
**2. Run the dashboard on a reachable address.** A non-loopback bind engages the OAuth gate, and the `client_id` just written activates the `nastechai` provider:
...
Like the NasTechai provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
Like the NasTechai provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
| **NasTechai Blue** (`nastechai-blue`) | NasTechai-branded blue accents with airy spacing |
