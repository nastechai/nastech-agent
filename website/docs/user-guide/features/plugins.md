...
**→ [Build a Nastech Plugin](/guides/build-a-nastech-plugin)** — step-by-step guide with a complete working example.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Nastech Plugin](/guides/build-a-nastech-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Nastech Plugin](/guides/build-a-nastech-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Nastech Plugin](/guides/build-a-nastech-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
See the **[full guide](/guides/build-a-nastech-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
