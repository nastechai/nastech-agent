...
Nastech has three hook systems that run custom code at key lifecycle points:
...
See the **[Build a Plugin guide](/guides/build-a-nastech-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Nastech will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
