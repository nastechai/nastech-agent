---
title: "Petdex — Install and select animated petdex mascots for Nastech"
sidebar_label: "Petdex"
description: "Install and select animated petdex mascots for Nastech"
---
...
Install and select animated petdex mascots for Nastech.
...
the Nastech CLI, TUI, and desktop app. This skill drives the `nastech pets` CLI
...
- Pillow (a core Nastech dependency) for sprite decoding — already installed.
...
Use the `terminal` tool to run `nastech pets <subcommand>`.

## Quick Reference

| Goal | Command |
| --- | --- |
| Browse the gallery | `nastech pets list` (add a substring to filter: `nastech pets list cat`) |
| List installed pets | `nastech pets list --installed` |
| Install a pet | `nastech pets install <slug>` (add `--select` to make it active) |
...
| Preview/animate in terminal | `nastech pets show [slug] [--cycle] [--state run]` |
| Disable the pet | `nastech pets off` |
| Remove a pet | `nastech pets remove <slug>` |
| Diagnose setup | `nastech pets doctor` |

## Procedure

1. Find a pet: `nastech pets list <query>` and note its `slug`.
2. Install + activate: `nastech pets install <slug> --select`.
3. Preview it: `nastech pets show` (Ctrl+C to stop).
...
- `scale` (float) — on-screen size of the native 192×208 frames (default 0.33,
  ...
  `nastech pets scale <factor>`, the `/pet scale` slash command, or the desktop
  Appearance slider.
...
- `nastech pets doctor` reports `✓ ready` when a pet is installed, selected,
