...
The GitHub webhook payload includes PR metadata (title, description, branch names, URLs) but **not the diff**. The prompt above instructs the agent to run `gh pr diff` to fetch the actual changes. The `terminal` tool is included in the default `nastech-webhook` toolset, so no extra configuration is needed.
:::
...
- **[Build a Plugin](/guides/build-a-nastech-plugin)** — package review logic into a shareable plugin
