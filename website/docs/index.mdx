...
description: "The self-improving AI agent built by Nastechai Research. A built-in learning loop that creates skills from experience, improves them during use, and remembers across sessions."
...
The self-improving AI agent built by [Nastechai Research](https://nastechairesearch.com). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
After installing, run `nastech setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [Nastechai Portal](/integrations/nastechai-portal).
:::
...
- **Built by model trainers** — Created by [Nastechai Research](https://nastechairesearch.com), the lab behind Nastech, Nomos, and Psyche. Works with [Nastechai Portal](https://portal.nastechairesearch.com), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [Nastechai Portal](/integrations/nastechai-portal) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [Nastechai Research](https://nastechairesearch.com) — the lab behind Nastech, Nomos, and Psyche models
