...
进入 **🎨 Image Generation**，选择后端（Nastechai Subscription 或 FAL.ai），随后在表格中用方向键选择模型，回车确认：
...
image_gen:
  ...
  use_gateway: false            # 使用 Nastechai Subscription 时为 true
```
...
3. **提交** — `_submit_fal_request()` 根据凭据走直连 FAL 或 Nastechai 托管网关。  
...
- **需要 FAL 凭据**（直连 `FAL_KEY` 或 Nastechai 订阅网关）  
