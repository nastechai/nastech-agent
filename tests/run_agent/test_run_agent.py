...
class TestBuildSystemPrompt:
    ...
    def test_includes_nastechai_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_nastechai_subscription_prompt", lambda tool_names: "NASTECHAI SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "NASTECHAI SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_nastechai_route(self, agent):
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.plugins.invoke_hook", _capture_hook)
...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_timeout_returns_finished_tools_without_hanging(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_CONCURRENT_TOOL_TIMEOUT_S", "0.1")
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "nastech_cli.plugins.get_pre_tool_call_block_message",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.plugins.invoke_middleware",
...
class TestRunConversation:
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "nastech_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("nastech_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.plugins.has_hook", _has_hook)
    ...
    def test_nastechai_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "nastechai"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_nastechai_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
...
class TestNastechaiCredentialRefresh:
    """Verify Nastechai credential refresh rebuilds the runtime client."""

    def test_try_refresh_nastechai_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "nastech_cli.auth.resolve_nastechai_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_nastechai_client_credentials(force=True)
