...
class TestBuildApiKwargsnastechaiPortal:
    def test_includes_nastechai_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == nastechai_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/nastechai) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_nastechai_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _nastechai_MODEL, get_text_auxiliary_client
        nastechai_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_nastechai_auth", return_value=nastechai_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("nastech_cli.models.get_nastechai_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == _nastechai_MODEL

    def test_custom_endpoint_when_no_nastechai(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/nastechai keys are available.
