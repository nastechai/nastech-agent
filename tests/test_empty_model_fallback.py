...
class TestGetDefaultModelForProvider:
    ...
    def test_nastechai_silent_default_is_not_the_expensive_flagship(self):
        """Nastechai Portal is a metered aggregator whose curated list is ordered
        ...
        result = get_default_model_for_provider("nastechai")
        assert result, "nastechai must resolve to a usable default model"
        assert "opus" not in result.lower(), (
            f"silent default escalated to an expensive flagship: {result!r}"
        )
        assert result != _PROVIDER_MODELS["nastechai"][0], (
        ...
        assert result == _PROVIDER_SILENT_DEFAULT_OVERRIDES["nastechai"]
        assert result in _PROVIDER_MODELS["nastechai"]
