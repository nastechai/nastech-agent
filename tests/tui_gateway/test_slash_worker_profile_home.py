...
def test_slash_worker_without_profile_home():
    ...
    with patch.dict("sys.modules", {
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            # Check that NASTECH_HOME was NOT overridden
            call_kwargs = mock_popen.call_args[1]
            assert "env" in call_kwargs
            # NASTECH_HOME should be from parent env or undefined (inherited from os.environ)
...
def test_slash_worker_with_none_profile_home():
    ...
    with patch.dict("sys.modules", {
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            # Check that NASTECH_HOME was NOT set
            call_kwargs = mock_popen.call_args[1]
            env = call_kwargs["env"]
            # When profile_home is None, NASTECH_HOME should come from parent env only
            if "NASTECH_HOME" in env:
