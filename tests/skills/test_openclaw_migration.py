...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `nastech cron` to recreate scheduled tasks" in notes_text
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Nastech remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "nastech prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Nastech was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "nastech uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Nastech said to ask Nastech about Nastech settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.nastech`` (the real Nastech home), not the broken
    ``.Nastech``.
    ...
    rewritten to ``~/.Nastech/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.nastech/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .nastech directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.nastech'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "nastech config path: ~/.nastech/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Nastech" in result
