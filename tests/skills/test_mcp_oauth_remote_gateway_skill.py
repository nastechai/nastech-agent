...
def test_nastech_home_env_fallback(tmp_path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "custom-home"))
    # Block the nastech_constants import so the env fallback is exercised
    with patch.dict(sys.modules, {"nastech_constants": None}):
        home = mod._nastech_home()
