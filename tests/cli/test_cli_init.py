...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"NASTECH_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"NASTECH_MAX_ITERATIONS": "not-a-number"})
...
class TestFallbackChainInit:
    def test_merges_new_and_legacy_fallback_config(self):
        cli = _make_cli(config_overrides={
            ...
            "fallback_model": {"provider": "nastechai", "model": "Nastech-4"},
        })
...
class TestHistoryDisplay:
    ...
    def test_resume_updates_nastech_session_id_env_and_context(self, tmp_path):
        ...
        os.environ["NASTECH_SESSION_ID"] = "current_session"
        _VAR_MAP["NASTECH_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["NASTECH_SESSION_ID"] == "target_session"
            assert get_session_env("NASTECH_SESSION_ID") == "target_session"
        finally:
