...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._nastech_home", tmp_path), \
             patch("nastech_constants.display_nastech_home", return_value="~/.nastech/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.nastech/profiles/coder/config.yaml`"
