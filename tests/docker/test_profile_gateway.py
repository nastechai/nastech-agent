...
def test_profile_delete_stops_gateway(
    ...
    _sh(container_name, f"nastech profile create {PROFILE}")
    _sh(container_name, f"nastech -p {PROFILE} gateway start", timeout=60)
    time.sleep(3)

    r = _sh(
        container_name,
        f"nastech profile delete {PROFILE} --yes",
        timeout=30,
    )
