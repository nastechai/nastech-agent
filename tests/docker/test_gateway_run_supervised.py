...
Before the s6 migration, ``docker run nastechai/nastech-agent gateway
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``nastech gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    start_container(
        ...
        "NASTECH_GATEWAY_NO_SUPERVISE=1",
        cmd="gateway run",
    )
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "NASTECH_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${NASTECH_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Nastech Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "Nastech Gateway
    ...
    assert "⚕" in combined or "Nastech Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Nastech Gateway Starting" in file_contents, (
