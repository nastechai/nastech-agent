...
def test_install_bws_rejects_malicious_member(nastech_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(nastech_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(nastech_home, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under nastech_home/cache/."""
    home = tmp_path / ".nastech"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
