...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from nastech_cli.debug import _upload_dpaste_com
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, nastech_home):
    ...
    def test_empty_primary_reports_file_empty(self, nastech_home):
    ...
    def test_truncates_large_file(self, nastech_home):
        """Files larger than max_bytes get tail-truncated."""
        from nastech_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (nastech_home / "logs" / "agent.log").write_text(big_content * 200)
    ...
    def test_drops_partial_when_truncation_mid_line(self, nastech_home):
    ...
    def test_unknown_log_returns_none(self, nastech_home):
        from nastech_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, nastech_home):
    ...
    def test_prefers_primary_over_rotated(self, nastech_home):
    ...
    def test_falls_back_when_primary_empty(self, nastech_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_agent_log(self, nastech_home):
        from nastech_cli.debug import collect_debug_report
    ...
    def test_report_includes_errors_log(self, nastech_home):
        from nastech_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, nastech_home):
        from nastech_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, nastech_home):
        from nastech_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, nastech_home):
        from nastech_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_survives_sweep_failure(self, nastech_home, capsys):
        ...
        with patch("nastech_cli.dump.run_dump"), \
             patch(
                 "nastech_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, nastech_home, capsys):
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, nastech_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from nastech_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("nastech_cli.dump.run_dump"), \
             patch("nastech_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("nastech_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, nastech_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, nastech_home, capsys):
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronastechaily on the next ``nastech debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, nastech_home):
        ...
        import ast
        import inspect
        from nastech_cli.debug import _schedule_auto_delete
        ...
        # (it should just write pending.json synchronastechaily).
        import os as _os
    ...
    def test_skips_non_paste_rs_urls(self, nastech_home):
    ...
    def test_merges_with_existing_pending(self, nastech_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, nastech_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, nastech_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_survives_sweep_failure(self, nastech_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: nastech debug" in out
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        ...
        with patch("nastech_cli.debug.delete_paste", return_value=True):
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, nastech_home, capsys):
        from nastech_cli.debug import run_debug_share
        ...
        with patch("nastech_cli.dump.run_dump"), \
             ...
             patch("nastech_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, nastech_home, capsys):
        from nastech_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    def test_returns_structured_urls(self, nastech_home):
        from nastech_cli.debug import build_debug_share, DebugShareResult
    ...
    def test_skips_missing_logs_without_failure(self, nastech_home):
        ...
        with patch("nastech_cli.dump.run_dump"), patch(
            "nastech_cli.debug.upload_to_pastebin",
    ...
    def test_required_report_failure_raises(self, nastech_home):
        from nastech_cli.debug import build_debug_share
...
# ---------------------------------------------------------------------------
# Shared bundle collection + Nastechai-S3 path
# ---------------------------------------------------------------------------

class TestCollectShareBundle:
    def test_returns_report_and_logs(self, nastech_home):
    ...
    def test_build_debug_share_uses_collector(self, nastech_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from nastech_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNastechaiBundle:
    def test_envelope_shape_and_gzip(self, nastech_home):
        import gzip
        import json as _json

        from nastech_cli.debug import build_nastechai_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_nastechai_bundle(files, redact=True)
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_nastechai_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugShareNastechai:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_nastechai_success_prints_view_url(self, nastech_home, capsys):
        from nastech_cli.debug import run_debug_share
        ...
        with patch("nastech_cli.dump.run_dump"), patch(
            "nastech_cli.diagnostics_upload.share_to_nastechai", return_value=res
        ) as share:
            run_debug_share(self._args())

        out = capsys.readouterr().out
        assert "Nastechai-INTERNAL" in out
        assert "https://support.example.com/diagnostics/id-1" in out
        assert "2026-06-20T00:00:00Z" in out
        # The blob passed to share_to_nastechai must be gzip bytes.
        blob = share.call_args[0][0]
        assert isinstance(blob, (bytes, bytearray)) and blob[:2] == b"\x1f\x8b"

    def test_nastechai_failure_suggests_local(self, nastech_home, capsys):
        from nastech_cli.debug import run_debug_share

        with patch("nastech_cli.dump.run_dump"), patch(
            "nastech_cli.diagnostics_upload.share_to_nastechai",
        ...
        assert "Nastechai upload failed" in err
        assert "--local" in err

    def test_nastechai_does_not_touch_pastebin(self, nastech_home):
        from nastech_cli.debug import run_debug_share
...
class TestDebugSlashCommand:
    """`/debug [nastechai|local]` parsing in the CLI/TUI handler.
    ...
    def test_bare_debug_defaults_to_paste(self):
        ...
        assert c["nastechai"] is False and c["local"] is False
    ...
    def test_nastechai_word_sets_nastechai(self):
        c = self._captured("/debug nastechai")
        assert c["nastechai"] is True and c["local"] is False

    def test_local_word_sets_local(self):
        c = self._captured("/debug local")
        assert c["local"] is True and c["nastechai"] is False

    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug NASTECHAI")
        assert c["nastechai"] is True

    def test_local_wins_over_nastechai(self):
        # local never touches the network, so it takes precedence.
        c = self._captured("/debug nastechai local")
        assert c["local"] is True and c["nastechai"] is False
...
class TestShareConsentGate:
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, nastechai=False,
                    no_redact=False, yes=False)
        base.update(over)
        return SimpleNamespace(**base)

    def test_aborts_on_user_decline(self, nastech_home, capsys, monkeypatch):
    ...
    def test_proceeds_on_user_accept(self, nastech_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, nastech_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, nastech_home, capsys, monkeypatch):
    ...
    def test_nastechai_path_also_gated(self, nastech_home, capsys, monkeypatch):
        """The --nastechai S3 path enforces the same consent gate (sibling site)."""
        from nastech_cli.debug import run_debug_share
        ...
        with patch("nastech_cli.dump.run_dump"), \
             patch("nastech_cli.diagnostics_upload.share_to_nastechai") as mock_nastechai:
            run_debug_share(self._args(nastechai=True))

        mock_nastechai.assert_not_called()
        assert "Aborted" in capsys.readouterr().out
