...
def test_managed_install_takes_precedence(tmp_path):
    """When NASTECH_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("nastech_cli.config.get_managed_system", return_value="NixOS"), \
         patch("nastech_cli.config.get_nastech_home", return_value=tmp_path):
        from nastech_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $NASTECH_HOME. With no code-scoped stamp, a host
    ...
    with patch("nastech_cli.config.get_managed_system", return_value=None), \
         patch("nastech_cli.config.get_nastech_home", return_value=home), \
         patch("nastech_cli.config._running_in_container", return_value=False):
        from nastech_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("nastech_cli.config.get_managed_system", return_value=None), \
         patch("nastech_cli.config.get_nastech_home", return_value=home), \
         patch("nastech_cli.config._running_in_container", return_value=True):
        from nastech_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $NASTECH_HOME keeps
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("nastech_cli.config.get_nastech_home", return_value=hh), \
         patch("nastech_constants.get_nastech_home", return_value=hh):
        buf = io.StringIO()
