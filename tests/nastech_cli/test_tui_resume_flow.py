...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["nastech", "-z", "hello", "--model", "gpt-test", "--provider", "openai"],
    )
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["nastech", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.plugins",
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nastechai/nastech-test", provider="nastechai", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["NASTECH_MODEL"] == "nastechai/nastech-test"
    assert env["NASTECH_INFERENCE_MODEL"] == "nastechai/nastech-test"
    assert env["NASTECH_TUI_PROVIDER"] == "nastechai"
    assert env["NASTECH_INFERENCE_PROVIDER"] == "nastechai"
