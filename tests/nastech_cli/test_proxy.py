...
def test_nastechai_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    assert cred.base_url == "https://inference-api.nastechairesearch.com/v1"
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    from nastech_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from nastech_cli.proxy.cli import cmd_proxy_list_providers
