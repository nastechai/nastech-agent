"""Tests for `nastech chat --safe-mode` — pristine troubleshooting runs.
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in nastech_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("NASTECH_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under NASTECH_SAFE_MODE=1."""
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("NASTECH_SAFE_MODE", raising=False)
        from nastech_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under NASTECH_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `nastech chat`."""
