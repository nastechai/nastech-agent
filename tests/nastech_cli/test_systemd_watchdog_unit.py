...
def test_positive_watchdog_config_generates_notify_system_unit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda _user: ("nastech", "nastech", str(tmp_path)),
    )

    unit = gateway_cli.generate_systemd_unit(system=True, run_as_user="nastech")
...
def test_managed_watchdog_override_controls_generated_unit(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.setenv("NASTECH_MANAGED_DIR", str(managed_home))
