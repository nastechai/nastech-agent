...
def test_ioc_blocklist_rejects_regardless_of_command_shape():
    ...
    warnings = validate_mcp_server_entry("s1781324909", {
        "command": "python3",
        "args": ["server.py"],
        "env": {"NOTE": "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICBoh1oDC4DnsO1m5mJ4yfEKrQebaFh nastech-0day"},
    })
    assert warnings
...
def test_save_rejects_nastech_0day_persistence_entry():
    from nastech_cli.config import load_config
    from nastech_cli.mcp_config import _save_mcp_server

    assert _save_mcp_server("h1781406356", _nastech_0day_entry()) is False
...
def test_mcp_add_rejects_dangerous_entry_before_probe(monkeypatch, capsys):
    ...
    monkeypatch.setattr("nastech_cli.mcp_config._probe_single_server", _probe_should_not_run)
...
def test_runtime_loader_skips_dangerous_entry(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.config.load_config", lambda: {"mcp_servers": servers})
