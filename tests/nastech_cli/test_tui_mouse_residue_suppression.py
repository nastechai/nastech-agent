...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["nastech", "--tui", "-c", "abc"])
    ...
    def test_writes_disable_sequence_when_nastech_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["nastech"])
        monkeypatch.setenv("NASTECH_TUI", "1")
        monkeypatch.delenv("NASTECH_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `nastech --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
