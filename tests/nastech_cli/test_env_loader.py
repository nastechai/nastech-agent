...
def test_project_env_overrides_stale_shell_values_when_user_env_missing(tmp_path, monkeypatch):
    ...
    loaded = load_nastech_dotenv(nastech_home=home, project_env=project_env)
...
def test_main_import_applies_user_env_over_shell_values(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "OPENAI_BASE_URL=https://new.example/v1\nNASTECH_INFERENCE_PROVIDER=custom\n",
        encoding="utf-8",
    )
    ...
    sys.modules.pop("nastech_cli.main", None)
    importlib.import_module("nastech_cli.main")

    assert os.getenv("OPENAI_BASE_URL") == "https://new.example/v1"
    assert os.getenv("NASTECH_INFERENCE_PROVIDER") == "custom"
...
def test_utf16_le_bom_env_loads_and_rewrites_clean_utf8(tmp_path, monkeypatch):
    ...
    home.mkdir()
    env_file = home / ".env"
    content = "NASTECH_TEST_KEY=hello_utf16\nSECOND_KEY=world\n"
    env_file.write_bytes(codecs.BOM_UTF16_LE + content.encode("utf-16-le"))

    monkeypatch.delenv("NASTECH_TEST_KEY", raising=False)
    ...
    assert loaded == [env_file]
    assert os.getenv("NASTECH_TEST_KEY") == "hello_utf16"
    assert os.getenv("SECOND_KEY") == "world"
    assert os.environ.get("\ufffd\ufffdNASTECH_TEST_KEY") is None
    _assert_clean_utf8_env_on_disk(env_file, first_key="NASTECH_TEST_KEY")
...
def test_leading_replacement_char_does_not_rewrite(tmp_path):
    ...
    env_file = tmp_path / ".env"
    raw = b"\xffNASTECH_TEST_KEY=should-not-rewrite\nSECOND_KEY=ok\n"
    env_file.write_bytes(raw)
