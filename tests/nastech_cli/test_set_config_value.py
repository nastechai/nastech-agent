...
from nastech_cli.config import set_config_value, config_command
...
class TestCatchAllPatterns:
    ...
    def test_case_insensitive(self, _isolated_nastech_home):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_nastech_home):
...
class TestConfigYamlRouting:
    ...
    def test_nested_key(self, _isolated_nastech_home):
        set_config_value("terminal.backend", "docker")
        config = _read_config(_isolated_nastech_home)
        assert "docker" in config
        assert "terminal" not in _read_env(_isolated_nastech_home)
...
class TestFalsyValues:
    ...
    def test_empty_string_routes_to_env(self, _isolated_nastech_home):
    ...
    def test_empty_string_routes_to_config(self, _isolated_nastech_home):
    ...
    def test_zero_routes_to_config(self, _isolated_nastech_home):
...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_structured_json(self, _isolated_nastech_home, capsys):
        set_config_value("terminal.backend", "docker")
        capsys.readouterr()
    ...
    def test_config_get_dotted_token_yaml_key(self, _isolated_nastech_home, capsys):
    ...
    def test_config_unset_removes_env_key(self, _isolated_nastech_home, capsys):
        set_config_value("OPENROUTER_API_KEY", "sk-test")
        assert "OPENROUTER_API_KEY=sk-test" in _read_env(_isolated_nastech_home)
        capsys.readouterr()

        args = argparse.Namespace(config_command="unset", key="OPENROUTER_API_KEY")
        config_command(args)

        assert "OPENROUTER_API_KEY=" not in _read_env(_isolated_nastech_home)
...
class TestSchemaValidation:
    ...
    def test_unknown_top_level_key_written_with_notice(self, _isolated_nastech_home, capsys):
        ...
        assert "totally_made_up_key" in _read_config(_isolated_nastech_home)

    def test_unknown_subkey_written_with_notice(self, _isolated_nastech_home, capsys):
    ...
    def test_platforms_container_is_accepted(self, _isolated_nastech_home, capsys):
    ...
    def test_gateway_platforms_nested_is_accepted(self, _isolated_nastech_home, capsys):
    ...
    def test_unknown_approvals_subkey_warns_but_writes(self, _isolated_nastech_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "not a recognized config key" in out
        assert "notarealkey" in _read_config(_isolated_nastech_home)

    def test_known_approvals_subkey_is_accepted(self, _isolated_nastech_home, capsys):
    ...
    def test_close_typo_suggests_correct_key(self, _isolated_nastech_home, capsys):
    ...
    def test_typoed_subkey_suggests_sibling(self, _isolated_nastech_home, capsys):
    ...
    def test_known_top_level_key_accepted(self, _isolated_nastech_home):
        """Sanity check: real config keys still work."""
        set_config_value("terminal.backend", "docker")
        content = _read_config(_isolated_nastech_home)
        assert "backend: docker" in content

    def test_known_platform_config_accepted(self, _isolated_nastech_home):
    ...
    def test_open_dict_mcp_servers_accepts_any_subkey(self, _isolated_nastech_home):
