...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (NASTECH_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.nastech as the "real home" — which is NOT equal
    # to the NASTECH_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    nastech_home = tmp_path / "classic"
    ...
    _write(nastech_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from nastech_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: NASTECH_HOME is set to a custom path that is NOT under
    # ~/.nastech/profiles/ — get_default_nastech_root() returns NASTECH_HOME
