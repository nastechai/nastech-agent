...
def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Nastech Agent")
    pdb.create_project(conn, name="Nastech Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["nastech-agent", "nastech-agent-2"]
...
def test_db_path_under_nastech_home():
    # Resolves under NASTECH_HOME (set by the autouse isolation fixture).
