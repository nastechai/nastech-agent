...
class TestGetHermesHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"NASTECH_HOME": "/custom/path"}):
            home = get_nastech_home()
...
class TestEnsureHermesHome:
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from nastech_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(profile_home)}):
            ensure_nastech_home()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path)}):
            ...
            # stands out at nastech startup before logging is configured)
            captured = capsys.readouterr()
            assert "nastech config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path)}):
            ...
            assert "nastech config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path)}):
            ...
            assert "nastech config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_with_internal_spaces(self, tmp_path):
        ...
        path = "/Users/paulo/Library/Application Support/nastech/keys/id_ed25519"
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path)}, clear=False):
    ...
    def test_save_env_value_readback_resave_is_idempotent(self, tmp_path):
        """nastech setup path: dotenv unquotes, then re-save must not grow quotes."""
        from dotenv import dotenv_values
...
class TestRemoveEnvValue:
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestEnvWriteDenylist:
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``NASTECH_*``) keep working. The denylist
