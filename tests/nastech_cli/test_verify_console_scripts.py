...
class TestVerifyConsoleScriptsInstalled:
    ...
    def test_triggers_reinstall_when_nastech_exe_missing(
        self, temp_pyproject, fake_scripts_dir
    ):
        (fake_scripts_dir / "nastech-agent.exe").write_bytes(b"fake")
        (fake_scripts_dir / "nastech-acp.exe").write_bytes(b"fake")
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("nastech_cli.main._is_windows", return_value=False), \
             patch("nastech_cli.main._run_quarantined_install") as mock_install:
            from nastech_cli.main import _verify_console_scripts_installed
    ...
    def test_load_console_script_names_reads_pyproject(self, temp_pyproject):
        from nastech_cli.main import _load_console_script_names

        names = _load_console_script_names()
        assert names == ["nastech", "nastech-agent", "nastech-acp"]
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("nastech_cli.main._is_windows", return_value=False), \
             patch("nastech_cli.main._run_quarantined_install") as mock_install, \
             patch("nastech_cli.main._verify_console_scripts_installed") as mock_verify:
