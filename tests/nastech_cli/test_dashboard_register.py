...
class TestHappyPath:
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://nastech.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://nastech.example.com/auth/callback"
        )
...
class TestCustomPortalPersistence:
    ...
    def test_explicit_custom_url_updates_existing_in_place(self, capsys):
        ...
        assert (
            saved["NASTECH_DASHBOARD_PORTAL_URL"] == "https://new-preview.example.com"
        )
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        saved = self._run(
            args=_ns(portal_url="https://portal.nastechairesearch.com"),
            portal="https://portal.nastechairesearch.com",
            existing_portal=None,
        )
        assert (
            saved["NASTECH_DASHBOARD_PORTAL_URL"] == "https://portal.nastechairesearch.com"
        )
...
class TestPublicUrlPersistence:
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert "/auth/callback" not in saved["NASTECH_DASHBOARD_PUBLIC_URL"]

    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://nastech.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["NASTECH_DASHBOARD_PUBLIC_URL"] == "https://nastech.example.com:8443"
