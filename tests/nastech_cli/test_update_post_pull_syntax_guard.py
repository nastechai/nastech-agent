...
def test_capture_head_sha_returns_none_on_git_failure(monkeypatch, tmp_path):
    ...
    assert nastech_main._capture_head_sha(["git"], tmp_path) is None
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="nastech_cli/config.py")

    ok, failing_path, error = nastech_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("nastech_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="nastech_cli/main.py")

    ok, failing_path, _ = nastech_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("nastech_cli/main.py")


def test_validate_critical_files_syntax_detects_break_in_web_server(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="nastech_cli/web_server.py")

    ok, failing_path, _ = nastech_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("nastech_cli/web_server.py")
...
def test_production_tree_passes_syntax_guard():
    ...
    ok, failing_path, error = nastech_main._validate_critical_files_syntax(repo_root)

    assert ok is True, (
        f"Critical-path file {failing_path} fails to parse on current main; "
        f"nastech update would brick users. Error: {error}"
    )
