...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_nastech_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
    ...
    entry = next(item for item in entries if item["source"] == "manual:nastech_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:nastech_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """nastech auth add qwen-oauth must set active_provider in auth.json.
    ...
    # pool entry from pool.add_entry() still present for nastech auth list
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from nastech_cli.auth_commands import auth_reset_command
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `nastech logout` should target configured Codex if auth has no active provider."""
    nastech_home = tmp_path / "nastech"
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (nastech_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((nastech_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from nastech_cli.auth import _reset_config_provider
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("nastech_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("nastech_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("nastech_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "nastech_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((nastech_home / "auth.json").read_text())
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `nastech auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "nastech"))
    nastech_home = tmp_path / "nastech"
    nastech_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `nastech auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((nastech_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`nastech auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.nastech/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.nastech/.env (not the shell), the
    ...
    (nastech_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (nastech_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `nastech auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `nastech auth remove`.  This is the gate that prevents shell-exported
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import nastech_cli.copilot_auth as ca
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `nastech auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `nastech auth add openai-codex`)
