...
# Ready for every zero-env-var provider row, including logged-out nastechai
...
def test_visible_providers_reuses_logged_out_feature_snapshot(monkeypatch):
    ...
    assert any(
        provider.get("managed_nastechai_feature") == "image_gen"
...
def test_visible_providers_reuses_pool_video_feature_snapshot(monkeypatch):
    ...
    account = nastechaiPortalAccountInfo(
        ...
        tool_access=nastechaiToolAccessInfo(
            enabled=True,
    ...
    assert not any(
        provider.get("managed_nastechai_feature") == "video_gen"
        for provider in providers
    )
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    """The bug: a frozen list never gained bfl, so composite users got nastechai
