...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("nastech_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_nastech_dev(container_env, monkeypatch):
    """Returns None when NASTECH_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("NASTECH_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_nastech_dev_zero(container_env, monkeypatch):
    """NASTECH_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("NASTECH_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        nastech_home = Path(tmpdir) / ".nastech"
        nastech_home.mkdir()
        (nastech_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("nastech_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_nastech_home": lambda: nastech_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NASTECH_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=nastech-custom\n"
        "exec_user=myuser\n"
        "nastech_bin=/opt/nastech/bin/nastech\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "nastech-custom"
    assert info["exec_user"] == "myuser"
    assert info["nastech_bin"] == "/opt/nastech/bin/nastech"
