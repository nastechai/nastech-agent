...
def test_chat_subcommand_yolo_flag_sets_env_before_startup(monkeypatch):
    """nastech chat --yolo must also set NASTECH_YOLO_MODE before
    _prepare_agent_startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["nastech", "chat", "--yolo"]
    )
    assert result == "1", (
        "NASTECH_YOLO_MODE was not '1' when _prepare_agent_startup was "
...
def test_no_yolo_flag_leaves_env_unset_at_startup(monkeypatch):
    """Without --yolo, NASTECH_YOLO_MODE must not be set at startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["nastech"]
    )
    assert result is None, (
        "NASTECH_YOLO_MODE was unexpectedly set at startup without --yolo."
    )
