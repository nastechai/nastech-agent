...
def test_removes_symlinks_pointing_into_nastech_node(fake_home):
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(nastech_home) == []


def test_removes_dangling_symlink_into_nastech_node(fake_home):
    """A link into the Nastech node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Nastech links that exist; ignores the ones that don't."""
    nastech_home = fake_home / ".nastech"
    node_bin = _make_nastech_node(nastech_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Nastech-managed; node is a real user binary.
