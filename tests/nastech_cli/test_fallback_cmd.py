    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nastechai", "model": "Nastech-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_skips_incomplete_entries(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nastechai", "model": "foo"},  # valid
                "not-a-dict",                          # noise
            ]
        }
        assert _read_chain(cfg) == [{"provider": "nastechai", "model": "foo"}]

    def test_returns_copies_not_aliases(self):
        from nastech_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "nastechai", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "nastechai"
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nastechai / Nastech-4")
