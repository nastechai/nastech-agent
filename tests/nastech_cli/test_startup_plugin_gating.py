...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["nastech", "-w", "--tui"], None),
        (["nastech", "version"], "version"),
        (["nastech", "--tui", "chat"], "chat"),
        (["nastech", "-w", "logs"], "logs"),
        (["nastech", "chat", "hello world"], "chat"),
        (["nastech", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["nastech", "-m", "gpt5", "chat"], "chat"),
        (["nastech", "--model", "gpt5", "chat", "hi"], "chat"),
        (["nastech", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["nastech", "-z", "hello world"], None),
        (["nastech", "-z", "hello", "chat"], "chat"),
        (["nastech", "--model=gpt5", "chat"], "chat"),     # inline form
        (["nastech", "--", "chat"], "chat"),               # -- terminator
        (["nastech", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["nastech", "some-plugin-cmd"], "some-plugin-cmd"),
        (["nastech", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["nastech"],                          # bare → chat
        ["nastech", "--help"],                # top-level help
        ["nastech", "-h"],
        ["nastech", "version"],               # known built-in
        ["nastech", "logs"],
        ["nastech", "gateway", "run"],
        ["nastech", "--tui"],
        ["nastech", "-w", "--tui"],
        ["nastech", "chat", "hi"],
        ["nastech", "help"],                  # accepted built-in-ish
        ["nastech", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["nastech", "meet", "join"],          # potential google_meet plugin
        ["nastech", "honcho", "status"],      # potential memory plugin
        ["nastech", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``nastech help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"nastech_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
