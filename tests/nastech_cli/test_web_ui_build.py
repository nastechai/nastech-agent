...
from nastech_cli.main import _web_ui_build_needed, _build_web_ui, _run_npm_install_deterministic
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        """Regression: sentinel must be in nastech_cli/web_dist/, NOT web/dist/."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        _touch(web_dir / "src" / "App.tsx", offset=-10)
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with patch("nastech_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("nastech_cli.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("nastech_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_idle:
            result = _build_web_ui(web_dir)
    ...
    def test_npm_install_uses_utf8_replace_output_decoding(self, tmp_path):
        ...
        with patch("nastech_cli.main.subprocess.run", return_value=mock_cp) as mock_run:
    ...
    def test_npm_install_fallback_forces_include_dev_and_no_save(self, tmp_path):
        ...
        with patch("nastech_cli.main.subprocess.run", side_effect=[ci_fail, install_ok]) as mock_run:
            result = _run_npm_install_deterministic("/usr/bin/npm", web_dir)
    ...
    def test_npm_install_uses_workspace_web_scope(self, tmp_path):
        ...
        with patch("nastech_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("nastech_cli.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("nastech_cli.main._run_with_idle_timeout", return_value=build_ok):
            result = _build_web_ui(web_dir)
        assert result is True
