...
def test_xai_oauth_authorize_url_includes_referrer_nastech_agent():
    """Attribution: xAI's OAuth server can identify Nastech-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "nastech-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "nastech auth add xai-oauth" in body
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    nastech_home = tmp_path / "nastech"
    _setup_nastech_auth(nastech_home, refresh_token="")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_nastech_auth(nastech_home, access_token=fresh)
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.setenv("NASTECH_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    nastech_home = tmp_path / "nastech"
    _seed_xai_oauth_state(nastech_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nastechai")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    assert raw["active_provider"] == "nastechai"
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    nastech_home = tmp_path / "nastech"
    _seed_xai_oauth_state(nastech_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    monkeypatch.setattr("nastech_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`nastech auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `nastech auth remove`.
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from nastech_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    nastech_home = tmp_path / "nastech"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_nastech_auth(nastech_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Nastech process refreshed the singleton
    ...
    Mirrors the proactive sync codex/nastechai already perform for the same
    reason, and is what makes the pool actually safe to share across
    profiles + Nastech processes."""
    from agent.credential_pool import load_pool

    nastech_home = tmp_path / "nastech"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_nastech_auth(nastech_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``nastech model`` -> xAI Grok OAuth (or another process
    ...
    cooldown elapses.  Mirrors the codex/nastechai self-heal pattern."""
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
    from dataclasses import replace

    nastech_home = tmp_path / "nastech"
    stale_at = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_nastech_auth(nastech_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    # Simulate the user re-running `nastech model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``nastech auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    Nastechai, etc.).  Users on xAI Grok OAuth would then see surprise charges
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    using it on the sync-back path means every xAI/Codex/Nastechai refresh in a
    ...
    assert raw_after["active_provider"] == "openrouter", (
        ...
        "Nastechai token rotations silently take over multi-provider users' "
