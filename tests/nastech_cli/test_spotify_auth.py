...
def test_store_provider_state_can_skip_active_provider() -> None:
    auth_store = {"active_provider": "nastechai", "providers": {}}
    ...
    assert auth_store["active_provider"] == "nastechai"
...
def test_resolve_spotify_runtime_credentials_refreshes_without_changing_active_provider(
    ...
    with auth_mod._auth_store_lock():
        ...
        store["active_provider"] = "nastechai"
        auth_mod._store_provider_state(
            store,
            "spotify",
            {
    ...
    assert auth_mod.get_active_provider() == "nastechai"
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes NASTECH_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    ...
    assert "NASTECH_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "NASTECH_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "NASTECH_SPOTIFY_CLIENT_ID" not in env_path.read_text()
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    Mirrors Nastechai / xAI-OAuth / Codex-OAuth / MiniMax quarantine pattern.
    """
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
