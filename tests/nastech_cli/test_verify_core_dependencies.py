...
class TestVerifyCoreDependencies:
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``nastech.exe`` on Windows before installing.
        ...
        is left missing and ``nastech`` drops off PATH. Previously this path
        ...
        with patch("nastech_cli.main._resolve_install_target_python", return_value=py), \
             patch("nastech_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("nastech_cli.main._is_windows", return_value=True), \
             patch("nastech_cli.main._venv_scripts_dir", return_value=fake_scripts), \
             patch("nastech_cli.main._run_install_with_heartbeat"), \
             patch("nastech_cli.main._quarantine_running_nastech_exe", return_value=[]) as mock_quar:
