...
def test_env_dist_with_index_starts_server(main_mod, monkeypatch, tmp_path):
    """A valid NASTECH_WEB_DIST (has index.html) proceeds to start_server
    without building."""
    _wire_common(main_mod, monkeypatch)
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "index.html").write_text("<html></html>", encoding="utf-8")
    monkeypatch.setenv("NASTECH_WEB_DIST", str(dist))
...
def test_env_dist_tilde_expanded_for_web_server(main_mod, monkeypatch, tmp_path):
    """A '~/...' NASTECH_WEB_DIST must be written back expanded so
    ...
    monkeypatch.setenv("NASTECH_WEB_DIST", "~/mydist")
    ...
    import os
    assert os.environ["NASTECH_WEB_DIST"] == str(dist)
