...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "nastech_cli" / "banner.py"
    ...
    with patch("nastech_cli.banner.subprocess.run") as mock_run, \
         patch("nastech_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    with patch("nastech_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/NastechaiResearch/nastech-agent.git\n")
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    with patch("nastech_cli.banner.subprocess.run") as mock_run:
        with patch("nastech_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when NASTECH_HOME has no git repo."""
    import nastech_cli.banner as banner
    ...
    # Point NASTECH_HOME at a temp dir with no nastech-agent/.git
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    with patch("nastech_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    NASTECH_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `nastech update` correctly refusing to run there. The guard
    ...
    with patch("nastech_cli.config.detect_install_method", return_value="docker"), \
         patch("nastech_cli.banner.subprocess.run") as mock_run, \
         patch("nastech_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("nastech_cli.config.detect_install_method", return_value="pip"), \
         patch("nastech_cli.banner.subprocess.run") as mock_run, \
         patch("nastech_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from nastech_cli.main import _invalidate_update_cache

    # Build a fake ~/.nastech with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"NASTECH_HOME": str(default_home)}):
