...
def test_recovery_self_lock_guard_clears_marker_without_install(tmp_path, monkeypatch):
    # Windows self-lock: nastech.exe is an ancestor of this Python process, so a
...
def test_recovery_self_lock_guard_inactive_when_not_ancestor(tmp_path, monkeypatch):
    # Windows, but nastech.exe is NOT in the ancestry (launched via `nastech
    ...
    class FakeProc:
        ...
        def parents(self):
            # Ancestry is plain pythons / cmd — no nastech.exe shim.
