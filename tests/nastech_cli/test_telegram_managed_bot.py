...
class TestGenerateBotUsername:
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("nastech_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestPollForToken:
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("nastech_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("nastech_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("nastech_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("nastech_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "nastech_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
