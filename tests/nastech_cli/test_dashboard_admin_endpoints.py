...
class TestCredentialPoolEndpoints:
    ...
    def test_delete_does_not_clobber_other_providers(self):
        ...
        from agent.credential_pool import load_pool
        from nastech_cli.auth import _load_auth_store, read_credential_pool
        from nastech_cli.config import save_env_value
...
class TestOpsEndpoints:
    ...
    def test_backup_output_uses_output_flag(self, monkeypatch):
        ...
        r = self.client.post(
            "/api/ops/backup",
            json={"output": "  /tmp/nastech-test.zip  "},
        )

        assert r.status_code == 200
        assert captured == {
            "subcommand": ["backup", "-o", "/tmp/nastech-test.zip"],
            "name": "backup",
        }
...
class TestToolsConfigEndpoints:
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_nastech_action", _fake_spawn)
        r = self.client.post(
