...
class TestShouldExclude:
    ...
    def test_includes_nested_nastech_agent_in_skills(self):
        """skills/autonomous-ai-agents/nastech-agent/ must NOT be excluded —
        only the root-level nastech-agent/ repo is skipped."""
        from nastech_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/nastech-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/nastech-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under NASTECH_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        # get_default_nastech_root needs this
    ...
    def test_excludes_nastech_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include nastech-agent/ directory."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        _make_nastech_tree(nastech_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "nastech-agent" in n]
            assert agent_files == [], f"nastech-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under NASTECH_HOME must be
        ...
        venv_pkg = nastech_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = nastech_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_nastech_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../nastech-agent/ but NOT root nastech-agent/."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        _make_nastech_tree(nastech_home)

        # Add a nested nastech-agent directory inside skills (like the real layout)
        nested = nastech_home / "skills" / "autonomous-ai-agents" / "nastech-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Nastech Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root nastech-agent must be excluded
            root_agent = [n for n in names if n.startswith("nastech-agent/")]
            assert root_agent == [], f"root nastech-agent leaked: {root_agent}"
            # Nested skill nastech-agent must be included
            assert "skills/autonomous-ai-agents/nastech-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/nastech-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/nastech-backup-*.zip."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        (nastech_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("nastech-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only nastech_state.db (old wrong name) is rejected."""
        from nastech_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["nastech_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into nastech home."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (nastech_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (nastech_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (nastech_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (nastech_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_nastech_prefix(self, tmp_path, monkeypatch):
        """Import strips .nastech/ prefix if all entries share it."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".nastech/config.yaml": "model: test\n",
            ".nastech/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (nastech_home / "config.yaml").read_text() == "model: test\n"
        assert (nastech_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_nastech_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a nastech backup."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside nastech home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (nastech_home / "gateway_state.json").write_text(live_state)
        ...
        assert (nastech_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (nastech_home / "config.yaml").exists()
        assert not (nastech_home / "gateway_state.json").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (nastech_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (nastech_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (nastech_home / "config.yaml").read_text() == "model: restored\n"
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without nastech markers fails validation."""
        import io
        from nastech_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_nastech(self):
        """Detects .nastech/ prefix wrapping all entries."""
        import io
        from nastech_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".nastech/config.yaml", "test")
            zf.writestr(".nastech/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".nastech/"
...
class TestBackupEdgeCases:
    def test_nonexistent_nastech_home(self, tmp_path, monkeypatch):
        """Backup exits when nastech home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".nastech"
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("nastech-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = nastech_home / "secret.db"
    ...
    def test_skips_output_zip_inside_nastech(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside nastech root."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        (nastech_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside nastech home
        out_zip = nastech_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        (nastech_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = nastech_home / "locked"
        locked_dir.mkdir()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (nastech_home / "profiles" / "coder" / "config.yaml").exists()
        assert (nastech_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "nastech -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    ...
    def test_creates_snapshot(self, nastech_home):
    ...
    def test_label_in_id(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", nastech_home=nastech_home)
    ...
    def test_copies_nested_files(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        assert (nastech_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_discord_recovery_ledger(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot

        gateway_dir = nastech_home / "gateway"
        gateway_dir.mkdir()
        ...
        copied = nastech_home / "state-snapshots" / snap_id / "gateway" / ledger.name
        assert copied.exists()
        conn = sqlite3.connect(copied)
        assert conn.execute("SELECT message_id FROM handled").fetchall() == [("123",)]
        conn.close()

    def test_copies_channel_aliases(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        copied = nastech_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        with open(nastech_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(nastech_home=empty) is None

    def test_max_file_size_skips_oversized_file(self, nastech_home, capsys):
        ...
        snap_id = create_quick_snapshot(
            nastech_home=nastech_home, max_file_size=cap
        )
        assert snap_id is not None
        snap_dir = nastech_home / "state-snapshots" / snap_id
    ...
    def test_max_file_size_none_copies_everything(self, nastech_home):
        """Default (no cap) preserves manual /snapshot behavior."""
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home, max_file_size=None)
        assert (nastech_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_max_file_size_under_cap_copies(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(
            nastech_home=nastech_home, max_file_size=1 << 30
        )
        assert (nastech_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_list_snapshots(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", nastech_home=nastech_home)
        id2 = create_quick_snapshot(label="second", nastech_home=nastech_home)

        snaps = list_quick_snapshots(nastech_home=nastech_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", nastech_home=nastech_home)
        snaps = list_quick_snapshots(limit=3, nastech_home=nastech_home)
        assert len(snaps) == 3

    def test_restore_config(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)

        (nastech_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (nastech_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, nastech_home=nastech_home)
        assert result is True
        assert "openrouter" in (nastech_home / "config.yaml").read_text()

    def test_restore_state_db(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)

        conn = sqlite3.connect(str(nastech_home / "state.db"))
    ...
    def test_restore_nonexistent(self, nastech_home):
        from nastech_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", nastech_home=nastech_home) is False

    def test_auto_prune(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", nastech_home=nastech_home)
        snaps = list_quick_snapshots(limit=100, nastech_home=nastech_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", nastech_home=nastech_home)
        deleted = prune_quick_snapshots(keep=3, nastech_home=nastech_home)
        assert deleted == 7
        assert len(list_quick_snapshots(nastech_home=nastech_home)) == 3
    ...
    def test_empty_pairing_dir_does_not_fail(self, nastech_home):
...
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, nastech_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, nastech_home):
        ...
        escape_dst = nastech_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped NASTECH_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_projects_db_snapshotted(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        copy = nastech_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        copy = nastech_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(nastech_home / "projects.db"))
    ...
    def test_additional_per_profile_dbs_round_trip(self, nastech_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(nastech_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, nastech_home):
        ...
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        snap = nastech_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
...
class TestPreUpdateBackup:
    ...
    def test_does_not_recurse_into_prior_backups(self, nastech_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from nastech_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(nastech_home=nastech_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(nastech_home=nastech_home)
    ...
    def test_rotation_preserves_manual_files(self, nastech_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from nastech_cli.backup import create_pre_update_backup

        (nastech_home / "backups").mkdir(exist_ok=True)
        manual = nastech_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(nastech_home=nastech_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from nastech_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(nastech_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, nastech_home):
        ...
        """
        from nastech_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(nastech_home=nastech_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, nastech_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, nastech_home):
        ...
        first = create_pre_update_backup(nastech_home=nastech_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(nastech_home=nastech_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(nastech_home=nastech_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    def nastech_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for nastech_cli.config + nastech_constants so they pick up NASTECH_HOME
    ...
    def _set_mode(nastech_home, value):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("nastech_cli.config"):
    ...
    def test_default_creates_quick_snapshot_only(self, nastech_home, capsys):
        ...
        assert self._snaps(nastech_home)
        assert not self._zips(nastech_home)

    def test_backup_flag_forces_full(self, nastech_home, capsys):
        ...
        assert "Saved:" in out
        assert "nastech import" in out
        assert len(self._zips(nastech_home)) == 1

    def test_no_backup_flag_skips_everything(self, nastech_home, capsys):
    ...
    def test_legacy_false_maps_to_off(self, nastech_home, capsys):
        """Legacy boolean ``false`` (the old zip opt-out) now means off."""
        self._set_mode(nastech_home, False)
        from nastech_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        assert snap_id is None
    ...
    def test_legacy_true_maps_to_full(self, nastech_home, capsys):
        """Legacy boolean ``true`` (the old always-zip opt-in) means full."""
        self._set_mode(nastech_home, True)
        from nastech_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_config_quick_mode(self, nastech_home, capsys):
        self._set_mode(nastech_home, "quick")
        from nastech_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_unknown_mode_falls_back_to_quick(self, nastech_home, capsys):
        self._set_mode(nastech_home, "bogus-mode")
        from nastech_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_no_backup_flag_overrides_full_config(self, nastech_home, capsys):
    ...
    def test_backup_flag_overrides_off_config(self, nastech_home, capsys):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, nastech_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `nastech
        # import` and the update-backup listing both pick them up.
        assert out.parent == nastech_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, nastech_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``nastech backup`` / ``create_pre_update_backup`` — no drift."""
        from nastech_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(nastech_home=nastech_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, nastech_home):
        from nastech_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(nastech_home=nastech_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(nastech_home=nastech_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, nastech_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(nastech_home=nastech_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((nastech_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_nastech_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.nastech yet — nothing to back up."""
        from nastech_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(nastech_home=missing)
        assert out is None
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_bom_live_file_still_counted(self, tmp_path):
        ...
        from nastech_cli.backup import _count_cron_jobs, restore_cron_jobs_if_emptied
        nastech_home = tmp_path / ".nastech"
        jobs_path = nastech_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(nastech_home)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = nastech_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, nastech_home=nastech_home) is None
        assert restore_cron_jobs_if_emptied("", nastech_home=nastech_home) is None
...
class TestMemoryProviderExternalPaths:
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        # External provider state living OUTSIDE NASTECH_HOME.
