...
def test_whatsapp_pairing_watcher_records_qr_and_connected():
    ...
    proc = _FakeProc([
        ...
        '{"event":"connected","user":{"id":"15551234567:1@s.whatsapp.net","name":"Nastech Bot"}}\n',
    ])
    ...
    assert record.account_name == "Nastech Bot"
...
def test_whatsapp_pairing_payload_includes_linked_account():
    ...
    assert payload["account_name"] == "Nastech Bot"
...
def test_spawn_whatsapp_pairing_process_uses_json_mode(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nastech_constants, "find_node_executable", lambda command: "/usr/bin/node")
    monkeypatch.setattr(nastech_constants, "with_nastech_node_path", lambda env=None: {})
