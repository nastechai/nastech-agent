"""Tests for the local-only Nastechai session classifier exposed on /api/status."""
...
from nastech_cli.auth import (
    NASTECHAI_SESSION_TERMINAL,
    NASTECHAI_SESSION_UNKNOWN,
    NASTECHAI_SESSION_VALID,
    get_nastechai_session_validity,
)
    ...
    return ".".join(
        (
            ...
            _encode(
                {
                    "sub": "test-user",
                    "scope": auth.DEFAULT_NASTECHAI_SCOPE,
...
def _block_live_auth(monkeypatch):
    monkeypatch.setattr(auth, "get_nastechai_auth_status", _fail_if_live_auth_is_used)
    monkeypatch.setattr(
        auth,
...
# ── get_nastechai_auth_status_local — refresh-free display snapshot ──
...
def test_local_status_not_logged_in_after_terminal_quarantine(monkeypatch):
    ...
    status = auth.get_nastechai_auth_status_local()
...
def test_local_status_repeated_polling_never_uses_live_auth(monkeypatch):
    ...
    assert all(
        auth.get_nastechai_auth_status_local()["logged_in"] for _ in range(10)
    )
