...
def test_change_flow_schedules_a_downgrade(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("change", "plus", "yes"), raising=False)
    monkeypatch.setattr(
...
def test_change_flow_upgrade_charges_now(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: st)
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("change", "ultra", "yes"), raising=False)
...
def test_change_menu_cancel_schedules_cancellation(cli, monkeypatch, capsys):
    cli._app = object()
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: _sub_state())
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("cancel_sub", "yes"), raising=False)
    seen = {}
...
def test_pending_change_menu_offers_undo(cli, monkeypatch, capsys):
    ...
    st = _sub_state(pending_downgrade_tier_name="Plus", pending_downgrade_at="2026-07-28")
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: st)
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("keep"), raising=False)
    called = {"n": 0}
...
def test_insufficient_scope_triggers_stepup_then_replays(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("change", "plus", "yes", "yes"), raising=False)
...
def test_unknown_preview_effect_fails_safe(cli, monkeypatch, capsys):
    ...
    cli._app = object()
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: _sub_state())
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("change", "plus"), raising=False)
