...
``is_nous_hermes_non_agentic`` should only match the actual Nastechai Research
...
@pytest.mark.parametrize(
    "model_name",
    [
        "nastechai/Hermes-3-Llama-3.1-70B",
        "nastechai/Hermes-3-Llama-3.1-405B",
        "hermes-3",
        "Hermes-3",
        "hermes-4",
        "hermes-4-405b",
...
def test_matches_real_nous_nastech_chat_models(model_name: str) -> None:
    ...
    assert _check_hermes_model_warning(model_name) == _NASTECH_MODEL_WARNING
...
def test_does_not_match_unrelated_models(model_name: str) -> None:
    assert not is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} NOT to be flagged as Nous Nastech 3/4"
    )
