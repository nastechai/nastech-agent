...
class TestSttCategory:
    ...
    def test_managed_row_shares_tts_coverage_category(self):
        from nastech_cli.nastechai_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY

        managed = [p for p in _stt_cat()["providers"] if p.get("managed_nastechai_feature")]
        assert managed, "expected a nastechai Subscription row"
        for p in managed:
            assert p["managed_nastechai_feature"] == "stt"
    ...
    def test_apply_provider_selection_stt(self):
        ...
        with patch(
            ...
            feats.return_value = MagicMock(
                nastechai_auth_present=False, account_info=None
            )
