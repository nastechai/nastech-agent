...
from nastech_cli.nastechai_account import (
    ...
    nastechai_portal_topup_url,
    reset_nastechai_portal_account_info_cache,
)
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "nastech-cli",
            "product_id": "nastechai-nastech-agent",
            "nastechai_client": "nastech-agent",
    ...
    info = get_nastechai_portal_account_info()
    ...
    assert info.product_id == "nastechai-nastech-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("nastech_cli.auth.resolve_nastechai_access_token", lambda: "fresh-token")
    monkeypatch.setattr("nastech_cli.nastechai_account._fetch_nastechai_account_info", lambda *a, **kw: payload)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    assert isinstance(info, NastechaiPortalAccountInfo)
    assert info.source == "account_api"
    assert info.fresh is True
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    ...
    assert info.credential_source == "pool:manual-nastechai"
    assert info.paid_service_access is None
...
def test_entitlement_message_returns_none_for_paid_access():
    info = NastechaiPortalAccountInfo(
        logged_in=True,
        source="account_api",
        fresh=True,
    ...
    assert format_nastechai_portal_entitlement_message(info, capability="paid models") is None
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Nastechai inference credentials are configured" in message
    assert "cannot verify your Nastechai Portal paid access" in message
    assert "Log in with `nastech model`" in message
...
def test_entitlement_message_for_no_subscription_or_credits():
    ...
    message = format_nastechai_portal_entitlement_message(info, capability="paid models")
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    message = format_nastechai_portal_entitlement_message(info, capability="Tool Gateway")

    assert message is not None
    assert "could not verify" in message
    assert "account_api_timeout" in message
    assert "Run `nastech model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a Nastechai Portal account or organisation" in message
...
def test_topup_url_is_org_pinned_when_slug_present():
    ...
    assert (
        nastechai_portal_topup_url(info)
...
def test_topup_url_falls_back_to_legacy_when_slug_null():
    ...
    url = nastechai_portal_topup_url(info)
...
def test_topup_url_defaults_to_production_portal_for_none():
    url = nastechai_portal_topup_url(None)
    assert url == "https://portal.nastechairesearch.com/billing?topup=open"
