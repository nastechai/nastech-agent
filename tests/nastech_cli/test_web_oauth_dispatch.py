...
def test_codex_dashboard_worker_persists_inside_session_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_mod,
        "_save_codex_tokens",
        lambda tokens: saved_homes.append(get_nastech_home()),
    )
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "nastech_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "nastech auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("nastech_cli.auth.get_auth_status", return_value={"logged_in": False}):
