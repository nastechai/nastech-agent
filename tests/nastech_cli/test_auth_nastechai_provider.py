...
def test_resolve_nastechai_runtime_credentials_invoke_jwt_is_idempotent(
    ...
    assert (
        payload["providers"]["nastechai"]["agent_key_obtained_at"]
...
def test_resolve_nastechai_runtime_credentials_reauths_when_invoke_scope_missing(
    ...
    payload = json.loads((nastech_home / "auth.json").read_text())
    assert payload["providers"]["nastechai"]["agent_key"] is None
...
class TestLoginNastechaiSkipKeepsCurrent:
    ...
    unconditionally after login, which flipped model.provider to "nastechai" while
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        assert auth_after["providers"]["nastechai"]["access_token"] == "fake-nastechai-token"
    ...
    def test_picking_model_switches_to_nastechai(self, tmp_path, monkeypatch):
        ...
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "nastechai"
...
def test_persist_nastechai_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    # providers.nastechai populated with the full state (new behaviour)
    singleton = payload["providers"]["nastechai"]
    ...
    # credential_pool.nastechai has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["nastechai"]
...
def test_persist_nastechai_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.nastechai and lets seeding
    ...
    # providers.nastechai reflects the latest write (singleton semantics)
    assert payload["providers"]["nastechai"]["access_token"] == second_token
    assert payload["providers"]["nastechai"]["agent_key"] == second_token
...
def test_persist_nastechai_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    # No "label" key embedded in providers.nastechai when the caller didn't supply one.
    payload = json.loads((nastech_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["nastechai"]
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    assert "nastech auth add nastechai" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    ...
    assert client.kwargs["headers"]["x-nastechai-refresh-token"] == "refresh-1"
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_mod.resolve_nastechai_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_NASTECHAI_PORTAL_URL]
...
class TestNastechaiDeviceAuthTimeoutMessage:
    ...
    def test_timeout_message_falls_back_to_default_portal(self):
        from nastech_cli.auth import (
            ...
            _nastechai_device_auth_timeout_message,
        )

        msg = _nastechai_device_auth_timeout_message("")
        assert f"{DEFAULT_NASTECHAI_PORTAL_URL.rstrip('/')}/login" in msg
...
def test_poll_for_token_timeout_raises_actionable_message():
    ...
    so both the CLI login and the dashboard poller (web_server._nastechai_poller,
...
def test_nastechai_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    def _timeout(**kwargs):
        raise TimeoutError(
            auth_mod._nastechai_device_auth_timeout_message(
