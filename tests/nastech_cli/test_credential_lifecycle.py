...
def test_delete_env_key_prunes_env_seeded_pool_entry(nastech_home):
    _write_env(nastech_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(nastech_home, _zai_pool_fixture())
...
def test_delete_env_key_removes_provider_pool_key_when_emptied(nastech_home):
...
def test_delete_survives_pool_reload(nastech_home):
...
def test_delete_unknown_key_404s(nastech_home):
...
def test_delete_does_not_touch_other_providers(nastech_home):
    _write_env(nastech_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    other_key = "dk-" + "e" * 24
...
def test_update_rotates_custom_provider_mirror(nastech_home):
    old = "sk-cp-" + "h" * 24
    new = "sk-cp-" + "i" * 24
    _write_env(nastech_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_update_leaves_unrelated_config_keys_alone(nastech_home):
    """A DIFFERENT key configured inline must not be rewritten by value-match."""
    old = "sk-un-" + "j" * 24
    unrelated = "sk-un-" + "k" * 24
    _write_env(nastech_home, OPENAI_API_KEY=old)
    _write_config(nastech_home, f"model:\n  provider: custom\n  api_key: {unrelated}\n")
...
def test_delete_scrubs_config_yaml_mirror(nastech_home):
    old = "sk-dl-" + "m" * 24
    _write_env(nastech_home, OPENAI_API_KEY=old)
    _write_config(nastech_home, f"model:\n  provider: custom\n  api_key: {old}\n")
...
def test_delete_then_resave_round_trip(nastech_home):
    _write_env(nastech_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(nastech_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    ...
    assert not is_source_suppressed("zai", "env:ZAI_API_KEY"), (
        "an explicit re-save must lift the suppression (like `nastech auth add`)"
    )
