from nastech_cli import nastechai_auth_keepalive as keepalive
...
def test_keepalive_refreshes_stale_pool_entry(monkeypatch):
    ...
    assert keepalive.refresh_nastechai_auth_keepalive_once() is True
...
def test_keepalive_falls_back_to_singleton_state(monkeypatch):
    ...
    def _resolve_nastechai_runtime_credentials(**kwargs):
        calls.append(kwargs)
        return {
            "provider": "nastechai",
    ...
    monkeypatch.setattr(
        ...
        _resolve_nastechai_runtime_credentials,
    )

    assert keepalive.refresh_nastechai_auth_keepalive_once(timeout_seconds=15.0) is True
