...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.nastech/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".nastech" / "plugins"
...
class TestResolvePreToolBlock:
    ...
    def test_no_directive_returns_none(self, monkeypatch):
        from nastech_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "nastech_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("nastech_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "nastech_test" / "plugins"
        # Set NASTECH_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With NASTECH_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("NASTECH_PLUGINS_DEBUG", "1")
        from nastech_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".nastech" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_HOME", str(prof))
        ctx = self._ctx()
