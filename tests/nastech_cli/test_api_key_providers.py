...
class TestProviderRegistry:
    ...
    def test_oauth_providers_unchanged(self):
        ...
        assert "nastechai" in PROVIDER_REGISTRY
        assert PROVIDER_REGISTRY["nastechai"].auth_type == "oauth_device_code"
...
PROVIDER_ENV_VARS = (
    ...
    "NASTECHAI_API_KEY", "GITHUB_TOKEN", "GH_TOKEN",
    "OPENAI_BASE_URL", "NASTECH_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
    ...
    def test_non_api_key_provider(self):
        status = get_api_key_provider_status("nastechai")
        assert status["configured"] is False
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_invalid_provider_raises(self):
        with pytest.raises(AuthError):
            resolve_api_key_provider_credentials("nastechai")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_counted_when_nastech_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Nastech has been explicitly configured."""
        import yaml
        from nastech_cli import config as config_module
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
