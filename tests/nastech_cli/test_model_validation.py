...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("nastech_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("nastech_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
...
class TestValidateApiFallback:
    ...
    def test_unknown_provider_soft_accepted_when_api_down(self):
        ...
        with patch("nastech_cli.models.provider_model_ids", return_value=[]):
