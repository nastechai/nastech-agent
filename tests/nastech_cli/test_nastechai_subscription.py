"""Tests for Nastechai subscription feature detection."""

from nastech_cli.nastechai_account import NastechaiPortalAccountInfo, NastechaiToolAccessInfo
from nastech_cli import nastechai_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> NastechaiPortalAccountInfo:
...
def _pool_account() -> NastechaiPortalAccountInfo:
    ...
    return NastechaiPortalAccountInfo(
        ...
        tool_access=NastechaiToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_nastechai_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_nastechai_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_nastechai is False
    assert features.web.direct_override is True
    assert features.web.current_provider == "exa"


def test_get_nastechai_subscription_features_force_fresh_forwards_account_request(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr(ns, "get_nastechai_portal_account_info", fake_account_info)
    ...
    features = ns.get_nastechai_subscription_features({}, force_fresh=True)
...
def test_get_nastechai_subscription_features_prefers_managed_modal_in_auto_mode(monkeypatch):
    monkeypatch.setattr("tools.tool_backend_helpers.managed_nastechai_tools_enabled", lambda: True)
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info", lambda: _account(logged_in=True, paid=True)
    )
    ...
    features = ns.get_nastechai_subscription_features(
        {"terminal": {"backend": "modal", "modal_mode": "auto"}}
    )

    assert features.modal.available is True
    assert features.modal.active is True
    assert features.modal.managed_by_nastechai is True
    assert features.modal.direct_override is False


def test_get_nastechai_subscription_features_marks_browser_use_as_managed_when_gateway_ready(monkeypatch):
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info", lambda: _account(logged_in=True, paid=True)
    )
    ...
    assert features.browser.managed_by_nastechai is True
    assert features.browser.direct_override is False
    assert features.browser.current_provider == "Browser Use"


def test_get_nastechai_subscription_features_uses_direct_browserbase_when_no_managed_gateway(monkeypatch):
    ...
    assert features.browser.available is True
    assert features.browser.active is True
    assert features.browser.managed_by_nastechai is False
    assert features.browser.direct_override is True
    assert features.browser.current_provider == "Browserbase"


def test_get_nastechai_subscription_features_prefers_camofox_over_managed_browser_use(monkeypatch):
...
def test_get_nastechai_subscription_features_requires_agent_browser_for_browserbase(monkeypatch):
    env = {
        "BROWSERBASE_API_KEY": "bb-key",
        "BROWSERBASE_PROJECT_ID": "bb-project",
    }
...
def test_get_nastechai_subscription_features_does_not_treat_quoted_false_as_gateway_opt_in(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}
...
def test_get_gateway_eligible_tools_ignores_quoted_false_opt_in(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info", lambda **kw: _account(logged_in=True, paid=True)
    )
    monkeypatch.setattr(
        ns,
...
def test_get_gateway_eligible_tools_pool_excludes_video(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_nastechai_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
        "_get_gateway_direct_credentials",
...
def test_get_gateway_eligible_tools_empty_when_not_entitled(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info", lambda **kw: _account(logged_in=True, paid=False)
    )
...
def test_prompt_enable_tool_gateway_paid_user_offers_video(monkeypatch):
    ...
    ns.prompt_enable_tool_gateway({"model": {"provider": "nastechai"}})

    blob = " ".join(captured["items"]).lower()
    assert "video" in blob


def test_apply_nastechai_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_nastechai_managed_defaults must write video_gen.provider and
    video_gen.use_gateway when a Nastechai subscriber selects video_gen
    without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_nastechai_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "nastechai"}}
    changed = ns.apply_nastechai_managed_defaults(
...
def test_apply_nastechai_managed_defaults_writes_image_gen_config(monkeypatch):
    """apply_nastechai_managed_defaults must write image_gen.use_gateway
    when a Nastechai subscriber selects image_gen without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_nastechai_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
...
def test_apply_nastechai_managed_defaults_skips_fal_tools_when_key_present(monkeypatch):
    """When FAL_KEY is set, apply_nastechai_managed_defaults should not touch
...
def test_apply_nastechai_managed_defaults_preserves_existing_video_gen_section(monkeypatch):
...
# ---------------------------------------------------------------------------
# ensure_nastechai_portal_access — inline login gate for `nastech tools`
# ---------------------------------------------------------------------------


def test_ensure_nastechai_portal_access_fast_path_when_already_paid(monkeypatch):
    ...
    monkeypatch.setattr(ns, "_run_nastechai_portal_login_only", _login)

    assert ns.ensure_nastechai_portal_access() is True
    assert login_called["v"] is False


def test_ensure_nastechai_portal_access_logs_in_then_grants(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info", lambda **kw: next(states),
    )
    monkeypatch.setattr(ns, "_run_nastechai_portal_login_only", lambda **kw: True)

    assert ns.ensure_nastechai_portal_access() is True


def test_ensure_nastechai_portal_access_returns_false_when_login_declined(monkeypatch):
    monkeypatch.setattr(
        ns, "get_nastechai_portal_account_info",
        lambda **kw: _account(logged_in=False, paid=None),
    )
    monkeypatch.setattr(ns, "_run_nastechai_portal_login_only", lambda **kw: False)

    assert ns.ensure_nastechai_portal_access() is False


def test_ensure_nastechai_portal_access_false_when_logged_in_but_unpaid(monkeypatch):
...
# ---------------------------------------------------------------------------
# STT — managed-by-Nastechai detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------

def test_stt_managed_by_nastechai_when_provider_openai_and_no_direct_key(monkeypatch):
    """Default `stt.provider: openai` with a Nastechai sub + no direct OpenAI key
    ...
    features = ns.get_nastechai_subscription_features({"stt": {"provider": "openai"}})

    assert features.stt.available is True
    assert features.stt.active is True
    assert features.stt.managed_by_nastechai is True
...
def test_stt_direct_key_overrides_managed(monkeypatch):
    ...
    assert features.stt.available is True
    assert features.stt.managed_by_nastechai is False
...
def test_stt_groq_provider_requires_groq_key(monkeypatch):
    ...
    features = ns.get_nastechai_subscription_features({"stt": {"provider": "groq"}})
...
def test_apply_nastechai_managed_defaults_flips_stt_provider_to_openai_for_nastechai_users(monkeypatch):
    """Fresh Nastechai-subscribed user with the DEFAULT_CONFIG `stt.provider: local`
    ...
    # Avoid the heavy real probing in get_nastechai_subscription_features.
    monkeypatch.setattr(
        ns,
        "get_nastechai_subscription_features",
        lambda config, **kw: ns.NastechaiSubscriptionFeatures(
            subscribed=True,
            nastechai_auth_present=True,
            provider_is_nastechai=True,
            account_info=_account(logged_in=True, paid=True),
            features={
                key: ns.NastechaiFeatureState(
                    key=key, label=key, included_by_default=True,
                    available=False, active=False, managed_by_nastechai=False,
    ...
    changed = ns.apply_nastechai_managed_defaults(config, enabled_toolsets=[])
...
def _stt_features_stub(*, account_info):
    return ns.NastechaiSubscriptionFeatures(
        subscribed=True,
        nastechai_auth_present=True,
        provider_is_nastechai=True,
...
def test_apply_nastechai_managed_defaults_keeps_local_stt_when_backend_works(monkeypatch):
...
def test_apply_nastechai_managed_defaults_skips_stt_when_not_entitled(monkeypatch):
...
def test_apply_nastechai_managed_defaults_skips_stt_when_groq_key_present(monkeypatch):
