...
def test_setup_syncs_nastechai_from_disk(tmp_path, monkeypatch):
    """Nastechai OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
    ...
    def fake_select():
        _write_model_config(tmp_path, "nastechai", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "nastechai"
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import nastech_cli.gateway as _gw_mod
...
def test_modal_setup_can_use_nastechai_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("nastech_cli.setup.managed_nastechai_tools_enabled", lambda: True)
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr(
        "nastech_cli.setup.get_nastechai_subscription_features",
        lambda config: type("Features", (), {"nastechai_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
...
def test_prompt_yes_no_returns_default_when_noninteractive_env_set(monkeypatch):
    """NASTECH_NONINTERACTIVE=1 (set by dashboard/desktop spawns) must make
    prompt_yes_no fall back to its default instead of reading stdin."""
    monkeypatch.setenv("NASTECH_NONINTERACTIVE", "1")
