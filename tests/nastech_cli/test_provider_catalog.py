...
def test_catalog_covers_every_nastech_model_provider():
    """PARITY CONTRACT: the catalog == the `nastech model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `nastech model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_nastech_model():
    """The catalog must not invent providers `nastech model` doesn't show."""
