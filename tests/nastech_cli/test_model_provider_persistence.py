...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        def _boom(path, data, **kwargs):
            ...
            assert data["model"]["provider"] == "nastechai"
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from nastech_cli.main import _model_flow_copilot
        from nastech_cli.config import load_config
        ...
        with patch(
            ...
            "nastech_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
    ...
    def test_named_custom_provider_preserves_explicit_api_mode(self, config_home):
        ...
        with patch("nastech_cli.auth._save_model_choice"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("nastech_cli.models.fetch_api_models", return_value=["gpt-5.4"]), \
             patch("nastech_cli.curses_ui.curses_radiolist", side_effect=OSError("no tty in test")), \
    ...
    def test_named_custom_provider_with_builtin_slug_persists_custom_prefix(
        ...
        with patch("nastech_cli.auth._save_model_choice"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("nastech_cli.models.fetch_api_models", return_value=["MiniMax-M3"]), \
             patch("nastech_cli.curses_ui.curses_radiolist", side_effect=OSError("no tty in test")), \
             patch("builtins.input", return_value="1"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from nastech_cli.main import _model_flow_copilot_acp
        from nastech_cli.config import load_config

        with patch(
            "nastech_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "nastech_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("nastech_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("nastech_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        with patch("nastech_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.nastech/.env"):
...
class TestZaiEndpointPicker:
    ...
    def test_select_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.main._prompt_provider_choice", return_value=0), \
    ...
    def test_select_coding_plan_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.main._prompt_provider_choice", return_value=2), \
             patch("nastech_cli.auth._prompt_model_selection", return_value="glm-5.2"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_select_china_endpoint(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.main._prompt_provider_choice", return_value=1), \
             patch("nastech_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_cancel_keeps_existing_base_url(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.main._prompt_provider_choice", return_value=None), \
             patch("nastech_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
