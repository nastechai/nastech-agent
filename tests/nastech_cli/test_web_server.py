...
class TestReloadEnv:
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Nastech env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestWebServerEndpoints:
    ...
    def test_get_status(self):
        ...
        assert "nastech_home" in data
        assert "active_sessions" in data
        assert data["can_update_nastech"] is True
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(nastech_state, "DEFAULT_DB_PATH", fake_db_path)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(nastech_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_nastech"] is False
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in nastech-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "nastech"
    ...
    def test_get_honcho_config_returns_safe_defaults(self, monkeypatch, tmp_path):
        ...
        assert data["docs_url"] == "https://docs.honcho.dev/v3/guides/integrations/nastech"
        ...
        assert fields["workspace"]["placeholder"] == "nastech"
        assert fields["aiPeer"]["placeholder"] == "nastech"
    ...
    def test_put_honcho_blank_text_clears_key(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_nastech_home() / "honcho.json").read_text(encoding="utf-8"))
        assert "workspace" not in cfg.get("hosts", {}).get("nastech", {})
    ...
    def test_put_honcho_partial_save_preserves_other_keys(self, monkeypatch, tmp_path):
        ...
        host = json.loads((get_nastech_home() / "honcho.json").read_text(encoding="utf-8"))["hosts"]["nastech"]
    ...
    def test_put_honcho_number_stored_as_native_number(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_nastech_home() / "honcho.json").read_text(encoding="utf-8"))
        assert cfg["hosts"]["nastech"]["dialecticMaxChars"] == 1200
        assert isinstance(cfg["hosts"]["nastech"]["dialecticMaxChars"], int)
    ...
    def test_put_honcho_first_save_merges_into_resolved_config(self, monkeypatch, tmp_path):
        ...
        global_path.write_text(
            json.dumps({"baseUrl": "https://kept.example", "hosts": {"nastech": {"workspace": "kept"}}}),
            encoding="utf-8",
        )
        ...
        assert resp.status_code == 200
        assert not (get_nastech_home() / "honcho.json").exists()
        cfg = json.loads(global_path.read_text(encoding="utf-8"))
        assert cfg["baseUrl"] == "https://kept.example"
        assert cfg["hosts"]["nastech"] == {"workspace": "kept", "peerName": "eri"}
    ...
    def test_put_honcho_updates_legacy_dot_form_host_block(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("NASTECH_HONCHO_HOST", "nastech_work")

        path = self._seed_local_honcho({"hosts": {"nastech.work": {"workspace": "w", "peerName": "eri"}}})
        ...
        assert set(hosts) == {"nastech.work"}
        assert hosts["nastech.work"] == {"workspace": "w", "peerName": "eri", "sessionStrategy": "per-repo"}
    ...
    def test_put_honcho_api_key_never_overwrites_oauth_token(self, monkeypatch, tmp_path):
        ...
        path = self._seed_local_honcho({"hosts": {"nastech": {"apiKey": "hch-at-oauth-token"}}})
        ...
        assert cfg["hosts"]["nastech"]["apiKey"] == "hch-at-oauth-token"
    ...
    def test_put_honcho_tolerates_null_hosts(self, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert json.loads(path.read_text(encoding="utf-8"))["hosts"]["nastech"]["workspace"] == "myws"
    ...
    def test_memory_provider_config_honors_profile_param(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        # The suite pins NASTECH_HONCHO_HOST=nastech; this test exercises
        # profile-driven host resolution, so drop the override explicitly.
        monkeypatch.delenv("NASTECH_HONCHO_HOST", raising=False)
        from nastech_constants import get_nastech_home
        ...
        worker_cfg.write_text(json.dumps({"hosts": {"nastech_worker": {"workspace": "kept"}}}), encoding="utf-8")
        ...
        own = json.loads((get_nastech_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_nastech_home() / "images" / "nope.png"
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_nastech_home() / "images"
    ...
    def test_update_nastech_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        assert "docker pull nastechai/nastech-agent:latest" in data["message"]
        ...
        assert any("docker pull nastechai/nastech-agent:latest" in line for line in status_data["lines"])

    def test_update_nastech_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import nastech_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn nastech update")
    ...
    def test_update_nastech_spawns_on_non_docker_install(self, monkeypatch):
        import nastech_cli.web_server as web_server
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "nastech-update"}
        assert calls == [(["update"], "nastech-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        resp = self.client.get("/api/actions/nastech-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "nastech-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["nastech-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["nastech-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/nastech-update/status?lines=3")
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `nastech model` but
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `nastech import` doesn't auto-abort at the overwrite prompt."""
        import nastech_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_nastech_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_nastech_home() / "backups"
        assert archive.name.startswith("nastech-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_nastech_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import nastech_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("NASTECH_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "nastech-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_nastech_action", fail_spawn)
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://nastech-agent.nastechairesearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        assert teams["docs_url"] == (
            "https://nastech-agent.nastechairesearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://nastech-agent.nastechairesearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Nastech Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("NastechDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "nastech_pair123_bot",
                "deep_link": "https://t.me/newbot/NastechSetupBot/nastech_pair123_bot",
                "qr_payload": "https://t.me/newbot/NastechSetupBot/nastech_pair123_bot",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "nastech_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/NastechSetupBot/nastech_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/NastechSetupBot/nastech_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "nastech_pair_ready_bot",
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``nastech gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "nastech_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/NastechSetupBot/nastech_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/NastechSetupBot/nastech_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "nastech_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "nastech_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/NastechSetupBot/nastech_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/NastechSetupBot/nastech_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "nastech_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/NastechSetupBot/nastech_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/NastechSetupBot/nastech_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/nastech_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/nastech"})
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`nastech serve` (NASTECH_SERVE_HEADLESS) must NOT serve the SPA even
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from nastech_cli.web_server import _apply_main_model_assignment
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `nastech model`), not the first curated paid entry."""
...
class TestBuildSchemaFromConfig:
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see nastech_cli/config.py).
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from nastech_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("nastech://")
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        named = nastech_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_nastech_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_nastech_for_default_profile(self):
        from nastech_constants import get_nastech_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "nastech setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/nastech/bin/nastech")
        ...
        if is_windows:
            assert lines == ["@echo off", "nastech -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/nastech/bin/nastech -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_nastech_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_nastech_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_nastech_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_nastech_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_nastech_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_nastech_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_nastech_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_nastech_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_nastech_home() / "profiles" / "builder"
        token = set_nastech_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_nastech_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import nastech_cli.skills_config as skills_config
        import nastech_cli.web_server as web_server
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_RESTART_DRAIN_TIMEOUT", "90")
...
class TestThemeBootstrapCSS:
    ...
    def test_serve_index_no_bootstrap_for_builtin_theme(self, tmp_path, monkeypatch):
        ...
        resp = client.get("/chat")
        assert resp.status_code == 200
        assert "nastech-theme-bootstrap" not in resp.text
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["NASTECH_TUI_DASHBOARD"] == "1"
        assert env["NASTECH_TUI_INLINE"] == "1"
        assert env["NASTECH_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_sets_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_PYTHON_SRC_ROOT", raising=False)
        monkeypatch.delenv("NASTECH_PYTHON", raising=False)
        monkeypatch.delenv("NASTECH_CWD", raising=False)
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
        ...
        assert env["NASTECH_PYTHON_SRC_ROOT"] == str(main_mod.PROJECT_ROOT)
        assert env["NASTECH_PYTHON"] == sys.executable
        assert env["NASTECH_CWD"] == os.getcwd()
    ...
    def test_resolve_chat_argv_replaces_invalid_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_PYTHON_SRC_ROOT", "/definitely/missing/nastech-src")
        monkeypatch.setenv("NASTECH_PYTHON", "/definitely/missing/python")
        monkeypatch.setenv("NASTECH_CWD", "/definitely/missing/cwd")
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_resolve_chat_argv_keeps_relative_python_under_tui_cwd(
        ...
        monkeypatch.setenv("NASTECH_CWD", str(tmp_path))
        monkeypatch.setenv("NASTECH_PYTHON", str(relative_python))
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
        ...
        assert env is not None
        assert env["NASTECH_PYTHON"] == str(relative_python)
    ...
    def test_resolve_chat_argv_falls_back_when_getcwd_is_missing(self, monkeypatch, tmp_path):
        ...
        assert env is not None
        assert env["NASTECH_CWD"] == str(tmp_path)

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_nastech_home
    ):
        import nastech_cli.main as main_mod

        config_path = Path(os.environ["NASTECH_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/nastech-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/nastech-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"nastech-ws-ok" in buf:
                    break
            assert b"nastech-ws-ok" in buf
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        NASTECH_TUI_SIDECAR_URL env var pointing back at /api/pub on the
...
class TestDesktopCronTicker:
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_nastech_home):
        import threading
        import cron.scheduler as sched
