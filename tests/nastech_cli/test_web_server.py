...
class TestNewEndpoints:
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from nastech_cli.nastechai_subscription import NastechaiSubscriptionFeatures

        calls = 0
        features = NastechaiSubscriptionFeatures(
            subscribed=False,
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Nastechai Subscription rows, xAI TTS without Grok OAuth, and
        ...
        assert by_name["Nastechai Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_nastechai_provider_reports_needs_nastechai_auth(self, monkeypatch):
        """Selecting a managed Nastechai row while logged out flags needs_nastechai_auth.
        ...
        (ensure_nastechai_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Nastechai OAuth flow.
        """
        from nastech_cli.nastechai_account import NastechaiPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Nastechai Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_nastechai_auth"] is True
