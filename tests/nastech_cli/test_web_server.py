...
class TestWebServerEndpoints:
    ...
    def test_set_model_main_nastechai_applies_gateway_defaults(self, monkeypatch):
        """Switching the main provider to Nastechai calls apply_nastechai_managed_defaults
        ...
        monkeypatch.setattr(ns, "apply_nastechai_managed_defaults", fake_apply)

        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nastechai", "model": "nastech-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["provider"] == "nastechai"
        assert data["gateway_tools"] == ["web"]
        assert called["force_fresh"] is True

    def test_set_model_main_non_nastechai_skips_gateway_defaults(self, monkeypatch):
        """Non-Nastechai providers must NOT trigger Tool Gateway auto-routing."""
        import nastech_cli.nastechai_subscription as ns

        def boom(*args, **kwargs):  # pragma: no cover - must not be called
            raise AssertionError("apply_nastechai_managed_defaults called for non-nastechai provider")

        monkeypatch.setattr(ns, "apply_nastechai_managed_defaults", boom)
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nastechai", "default": "nastech-4"}
        cfg["auxiliary"] = {
            # Pinned to nastechai — same as the OLD main, becomes stale after switch.
            "compression": {"provider": "nastechai", "model": "anthropic/claude-sonnet-4.6"},
        ...
        assert comp["provider"] == "nastechai"
    ...
    def test_recommended_default_nastechai_honors_free_tier(self, monkeypatch):
        """For a free-tier Nastechai user, the recommended default must be a free
        model (mirroring `nastech model`), not the first curated paid entry."""
        import nastech_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_nastechai_model_ids", lambda: ["paid/expensive", "free/cheap"])
        monkeypatch.setattr(
            models_mod, "get_pricing_for_provider",
            lambda provider: {"paid/expensive": {"input": "1"}, "free/cheap": {"input": "0"}},
        )
        monkeypatch.setattr(models_mod, "check_nastechai_free_tier", lambda *, force_fresh=False: True)
        monkeypatch.setattr(
        ...
        resp = self.client.get("/api/model/recommended-default?provider=nastechai")
        assert resp.status_code == 200
        data = resp.json()
        assert data["provider"] == "nastechai"
    ...
    def test_recommended_default_nastechai_paid_uses_curated_default(self, monkeypatch):
        """A paid Nastechai user gets the first curated/paid-augmented model."""
        import nastech_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_nastechai_model_ids", lambda: ["top/model", "other/model"])
        monkeypatch.setattr(models_mod, "get_pricing_for_provider", lambda provider: {})
        monkeypatch.setattr(models_mod, "check_nastechai_free_tier", lambda *, force_fresh=False: False)
        monkeypatch.setattr(
    ...
    def test_recommended_default_handles_failure_gracefully(self, monkeypatch):
        ...
        monkeypatch.setattr(models_mod, "get_curated_nastechai_model_ids", boom)
