...
class TestInactivityTimeout:
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """NASTECH_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """NASTECH_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("NASTECH_CRON_TIMEOUT", "abc")
        raw = os.getenv("NASTECH_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """NASTECH_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("NASTECH_CRON_TIMEOUT", "")
        raw = os.getenv("NASTECH_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_nastech_constants_importable(self):
        """nastech_constants should be importable from cron context."""
        from nastech_constants import get_nastech_home
        assert callable(get_nastech_home)
