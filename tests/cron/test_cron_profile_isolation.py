...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_nastech_home", None, raising=False)
    assert scheduler._get_nastech_home().resolve() == profile_home.resolve()
    assert scheduler._get_nastech_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (NASTECH_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.NASTECH_DIR.resolve() == root.resolve()
