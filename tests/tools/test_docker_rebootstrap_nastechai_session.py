"""Unit tests for scripts/docker_rebootstrap_nastechai_session.py.

The boot-time re-seed is the load-bearing "does not clobber a healthy session"
guard: it may overwrite the on-disk nastechai provider entry when that entry is
...
_SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "docker_rebootstrap_nastechai_session.py"
_spec = importlib.util.spec_from_file_location("docker_rebootstrap_nastechai_session", _SCRIPT)
mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(mod)


def _terminal_nastechai_state():
...
def test_timezone_less_local_timestamp_is_incomparable(tmp_path):
    auth = _write_auth(tmp_path, {"nastechai": {
        **_healthy_nastechai_state(),
...
def test_terminal_entry_missing_marker_is_not_terminal(tmp_path):
    ...
    auth = _write_auth(tmp_path, {"nastechai": {"client_id": "nastech-cli-vps"}})
    assert mod.reseed_if_terminal(auth, _FRESH_SEED) == "not_terminal"
