...
the sync wire contract):
  ...
  * the access gate (nastechai admin) making sync inert
...
# ---------------------------------------------------------------------------
# Access gate (nastechai admin) + per-skill opt-in
# ---------------------------------------------------------------------------
...
class TestDevGate:
    def test_gate_open_with_claim(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ssc, "resolve_nastechai_runtime_credentials",
        ...
        monkeypatch.setattr(auth_mod, "resolve_nastechai_runtime_credentials",
                            lambda **kw: {"api_key": token, "base_url": "https://x"})
        ident = ssc.resolve_identity()
        assert ident["nastechai_admin"] is True
    ...
    def test_gate_closed_without_claim(self, monkeypatch):
        ...
        monkeypatch.setattr(auth_mod, "resolve_nastechai_runtime_credentials",
                            lambda **kw: {"api_key": token, "base_url": "https://x"})
        ident = ssc.resolve_identity()
        assert ident["nastechai_admin"] is False
    ...
    def test_maybe_pull_inert_when_not_logged_in(self, monkeypatch):
        ...
        monkeypatch.setattr(auth_mod, "resolve_nastechai_runtime_credentials", _raise)
...
def _org_identity(role=None, org_id="org-1", owner="owner1"):
    ...
    return {"api_key": token, "base_url": "http://x", "owner": owner,
            "nastechai_admin": True, "claims": claims,
