...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_nastech_bin(self):
        """After _INSTALL_FAILED, manual install in NASTECH_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        nastech_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(nastech_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(nastech_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._nastech_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == nastech_bin
            assert _tirith_mod._resolved_path == nastech_bin
            mock_clear.assert_called_once()
...
class TestNastechHomeIsolation:
    ...
    def test_failure_marker_respects_nastech_home(self):
        """_failure_marker_path must use NASTECH_HOME, not hardcoded ~/.nastech."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"NASTECH_HOME": "/custom/nastech"}):
            result = _failure_marker_path()
        assert result == "/custom/nastech/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets NASTECH_HOME; verify it's active."""
        nastech_home = os.getenv("NASTECH_HOME")
        assert nastech_home is not None, "NASTECH_HOME should be set by conftest"
        assert "nastech_test" in nastech_home, "Should point to test temp dir"

    def test_get_nastech_home_fallback(self):
        """Without NASTECH_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_nastech_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove NASTECH_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".nastech")
            result = _get_nastech_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._nastech_bin_dir",
