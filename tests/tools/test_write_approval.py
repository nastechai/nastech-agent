...
def test_memory_gate_on_no_interactive_stages(nastech_home):
...
def test_memory_gate_on_then_apply(nastech_home):
...
def test_skill_gate_off_allows_create(nastech_home):
...
def test_skill_gate_on_always_stages(nastech_home):
...
def test_skill_gate_on_then_apply_writes_file(nastech_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's NASTECH_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(nastech_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(nastech_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(nastech_home):
...
def test_handle_reject(nastech_home):
    from nastech_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_mode_alias_still_works(nastech_home):
...
def test_handle_approval_invalid(nastech_home):
    from nastech_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(nastech_home):
    from nastech_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_callback_error_stages(nastech_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(nastech_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(nastech_home, approval_callback_cleanup):
