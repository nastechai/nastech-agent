...
class TestYoloMode:
    ...
    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for NASTECH_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("NASTECH_YOLO_MODE", "")
        monkeypatch.setenv("NASTECH_INTERACTIVE", "1")
        monkeypatch.setenv("NASTECH_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("NASTECH_YOLO_MODE") returns ""
