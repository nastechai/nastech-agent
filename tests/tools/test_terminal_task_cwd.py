...
def test_stale_env_cwd_from_different_session_is_ignored(monkeypatch):
    ...
    class FakeEnv:
        env = {}
        cwd = "/home/user/src/nastech-desktop-tipc/apps/desktop"
    ...
    monkeypatch.setattr(terminal_tool, "_last_activity", {})
    monkeypatch.setattr(terminal_tool, "_task_env_overrides", {})
    monkeypatch.setattr(terminal_tool, "_get_env_config", lambda: _minimal_terminal_config(cwd="/home/user/src/nastech-agent"))
    monkeypatch.setattr(terminal_tool, "_start_cleanup_thread", lambda: None)
    ...
    assert result["exit_code"] == 0
    # The command must run in the config cwd (nastech-agent), NOT the stale
    # env.cwd left by session A (nastech-desktop-tipc).
    assert calls == [("pwd", {"timeout": 60, "cwd": "/home/user/src/nastech-agent", "bounded_capture": True})]
