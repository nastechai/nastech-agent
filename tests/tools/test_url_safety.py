...
class TestGlobalAllowPrivateUrls:
    ...
    def test_env_var_true(self, monkeypatch):
        """NASTECH_ALLOW_PRIVATE_URLS=true enables the toggle."""
    ...
    def test_env_var_1(self, monkeypatch):
        """NASTECH_ALLOW_PRIVATE_URLS=1 enables the toggle."""
        monkeypatch.setenv("NASTECH_ALLOW_PRIVATE_URLS", "1")
        assert _global_allow_private_urls() is True

    def test_env_var_yes(self, monkeypatch):
        """NASTECH_ALLOW_PRIVATE_URLS=yes enables the toggle."""
        monkeypatch.setenv("NASTECH_ALLOW_PRIVATE_URLS", "yes")
        assert _global_allow_private_urls() is True

    def test_env_var_false(self, monkeypatch):
        """NASTECH_ALLOW_PRIVATE_URLS=false keeps it disabled."""
        monkeypatch.setenv("NASTECH_ALLOW_PRIVATE_URLS", "false")
        assert _global_allow_private_urls() is False
...
class TestAllowPrivateUrlsIntegration:
    ...
    def test_benchmark_ip_allowed_when_toggle_on(self, monkeypatch):
        ...
        with patch("socket.getaddrinfo", return_value=[
            ...
            assert is_safe_url("https://nastechairesearch.com") is True
