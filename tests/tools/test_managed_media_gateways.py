...
@pytest.fixture(autouse=True)
def _enable_managed_nastechai_tools(monkeypatch):
    """Patch the source modules so managed_nastechai_tools_enabled() returns True
...
def test_managed_fal_submit_uses_gateway_origin_and_nastechai_token(monkeypatch):
    captured = {}
    ...
    assert captured["client_key"] == "nastechai-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "nastechai.com")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "nastechai-token")
    ...
    assert captured["api_key"] == "nastechai-token"
    assert captured["base_url"] == "https://openai-audio-gateway.nastechai.com/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.nastechai.com/v1"
