...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "nastechai.com",
        },
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_nastechai_tools_enabled", return_value=True):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "nastechai-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.nastechai.com"
    assert result.nastechai_user_token == "nastechai-token"
...
def test_resolve_managed_tool_gateway_is_inactive_without_nastechai_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "nastechai.com",
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "nastechai.com"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_nastechai_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "nastechai-token",
        )
...
def test_read_nastechai_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth.resolve_nastechai_access_token",
        lambda refresh_skew_seconds=120: "fresh-token",
    )

    assert managed_tool_gateway.read_nastechai_access_token() == "fresh-token"
...
def test_managed_vendor_endpoints_pin_the_deployed_gateway_url():
    ...
    with patch.dict(
        os.environ,
        {"TOOL_GATEWAY_DOMAIN": "nastechai.com", "TOOL_GATEWAY_SCHEME": "https"},
        clear=False,
    ):
    ...
    assert endpoints == {
        "origin": "https://tool-gateway.nastechai.com",
        "base_url": "https://tool-gateway.nastechai.com/api/bfl",
...
def test_managed_vendor_endpoints_do_not_consult_entitlement():
    ...
    assert endpoints["base_url"] == "https://tool-gateway.nastechai.com/api/bfl"
...
def test_managed_gateway_auth_headers_carry_the_bearer():
    with patch.object(managed_tool_gateway, "managed_nastechai_tools_enabled", return_value=True):
    ...
    assert headers == {"Authorization": "Bearer nastechai-token"}


def test_managed_gateway_auth_headers_reflect_a_rotated_token():
    # Read fresh on every call: a nastechai access token expires within the hour,
...
class TestManagedMediaUploader:
    """The presign -> PUT -> ``nastechai-upload:<token>`` protocol.
    ...
    def _uploader(self, **kwargs):
        return managed_tool_gateway.build_managed_media_uploader(
            ...
            token_reader=kwargs.pop("token_reader", lambda: "nastechai-token"),
        )
    ...
    def test_presign_declares_the_exact_type_and_length_the_put_then_sends(self):
        ...
        assert calls["presign"][0]["headers"]["Authorization"] == "Bearer nastechai-token"
        ...
        assert calls["result"] == "nastechai-upload:tok-1"
    ...
    def test_a_missing_credential_fails_before_any_request(self):
        ...
        with patch.object(managed_tool_gateway, "managed_nastechai_tools_enabled", return_value=True), \
            ...
            with pytest.raises(RuntimeError, match="no nastechai credential"):
