...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("nastech_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("nastech_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestDetectDangerousRm:
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            ...
            "rm -f /tmp/nested/../nastech-verify-example.py",
            "rm -f /tmp/./nastech-verify-example.py",
            "rm -f /tmp//nastech-verify-example.py",
...
class TestTeePattern:
    ...
    def test_tee_nastech_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.nastech/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_nastech_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $NASTECH_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_nastech_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$NASTECH_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestNastechConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.nastech/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.nastech/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.nastech/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.nastech/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.nastech/config.yaml")
        assert dangerous is True
        assert "nastech config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.nastech/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_nastech_home_config(self):
    ...
    def test_sed_in_place_absolute_nastech_home_env(self):
    ...
    def test_custom_nastech_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $NASTECH_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.nastech/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_nastech_home_config(self):
        config_path = get_nastech_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.nastech/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_nastech_home_env(self):
        env_path = get_nastech_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.nastech/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.nastech/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.nastech/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.nastech/config.yaml"
        )
        assert dangerous is True
        assert key != "in-place edit of Nastech config/env (perl/ruby)"

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.nastech/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Nastech config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_nastech_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $NASTECH_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    def test_cp_to_nastech_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.nastech/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    ...
    def test_windows_nastech_home_config_folds(self, monkeypatch):
        # Nastech home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("NASTECH_HOME", r"C:\Users\tester\.nastech")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.nastech\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(
        ...
        monkeypatch.setenv("NASTECH_LANGUAGE", lang)
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m nastech_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m nastech_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "nastech_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_nastech_gateway_stop_detected(self):
        cmd = "nastech gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_nastech_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `nastech` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "nastech -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_nastech_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "nastech --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_nastech_gateway_multiple_flags_detected(self):
        cmd = "nastech -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_nastech_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "nastech -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_nastech_gateway_start_not_flagged(self):
        cmd = "nastech gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_nastech_detected(self):
        """pkill targeting nastech/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_nastech_detected(self):
        cmd = "killall nastech"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Nastech security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.nastech/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    ...
    def test_pkill_nastech_still_detected(self):
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_stop_nastech_detected(self):
        cmd = "launchctl stop ai.nastech.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "nastech" in desc.lower()

    def test_launchctl_kickstart_nastech_detected(self):
        cmd = "launchctl kickstart -k system/ai.nastech.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_nastech_detected(self):
        cmd = "launchctl bootout system/ai.nastech.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_nastech_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.nastech.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Nastech label is out of scope for the
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("nastech_cli.config.load_config", return_value=cfg):
