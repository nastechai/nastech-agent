...
class TestPayloadFilters:
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_nastech_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.nastech/scripts paths must resolve through NASTECH_HOME for profiles."""
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.nastech/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
