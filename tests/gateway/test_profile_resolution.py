...
class TestResolutionOrder:
        ...
        with patch("nastech_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("nastech_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("nastech_cli.profiles.profile_exists", return_value=True):
                    ...
                    assert result == Path("/nastech/profiles/routed")
    ...
    def test_active_profile_fallback(self, mock_runner, discord_source):
        ...
        with patch("nastech_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("nastech_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/nastech/profiles/active")
                ...
                assert result == Path("/nastech/profiles/active")
    ...
    def test_default_fallback_when_no_active(self, mock_runner, discord_source):
        ...
        with patch("nastech_cli.profiles.get_active_profile_name", return_value=None):
            with patch("nastech_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/nastech")
...
class TestMissingProfileWarning:
    ...
    def test_existing_profile_no_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("nastech_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("nastech_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/nastech/profiles/existing")
                with patch("nastech_cli.profiles.profile_exists", return_value=True):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
                        
                        assert result == Path("/nastech/profiles/existing")
...
class TestExceptionHandling:
    ...
    def test_exception_with_no_profile_name(self, mock_runner, discord_source, caplog):
        ...
        with patch("nastech_cli.profiles.get_active_profile_name", return_value=None):
            with patch("nastech_cli.profiles.get_profile_dir", side_effect=RuntimeError("Filesystem error")):
                with patch("nastech_constants.get_nastech_home", return_value=Path("/nastech")):
                    mock_runner._profile_name_for_source = MagicMock(return_value=None)
