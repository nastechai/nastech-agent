...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("NASTECH_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("NASTECH_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.nastech/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_nastech_credentials(self, tmp_path, monkeypatch):
        """~/.nastech/.env and ~/.nastech/auth.json stay blocked even in
    ...
    def test_denylist_blocks_nastech_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        nastech_dir = fake_home / ".nastech"
        nastech_dir.mkdir(parents=True)
        config_file = nastech_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_nastech_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Nastech root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".nastech" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        nastech_root = fake_home / ".nastech"
        config_file = nastech_root / "config.yaml"
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.nastech/pairing/ (platform pairing tokens) are
    ...
    def test_nastech_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        nastech_dir = fake_home / ".nastech"
        cache_dir = nastech_dir / "cache" / "documents"
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``NASTECH_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("NASTECH_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_nastech_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.nastech/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        nastech_root = denied_root / ".nastech"
        prof_dir = nastech_root / "profiles" / "myprof"
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"NASTECH_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NASTECH_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"NASTECH_HUMAN_DELAY_MODE": "natural"}):
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            ...
            "NASTECH_HUMAN_DELAY_MIN_MS": "100",
            "NASTECH_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
