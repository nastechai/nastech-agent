...
async def test_goal_verdict_done_sent_via_adapter_send(nastech_home):
    ...
    with patch("nastech_cli.goals.judge_goal", return_value=("done", "the feature shipped", False, None, False)):
...
async def test_goal_verdict_survives_adapter_without_send(nastech_home):
    ...
    with patch("nastech_cli.goals.judge_goal", return_value=("done", "ok", False, None, False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
