...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        # `nastech update --gateway` against the CI checkout. The managed-install
        ...
        assert "brew upgrade nastech-agent" in result
    ...
    @pytest.mark.asyncio
    async def test_no_nastech_binary(self, tmp_path):
        """Returns error when nastech is not on PATH and nastech_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "nastech update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m nastech_cli.main when nastech not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m nastech_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "nastech_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_nastech_bin_prefers_which(self, tmp_path):
        """_resolve_nastech_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_nastech_bin

        with patch("shutil.which", return_value="/custom/path/nastech"):
            result = _resolve_nastech_bin()

        assert result == ["/custom/path/nastech"]
    ...
    @pytest.mark.asyncio
    async def test_resolve_nastech_bin_returns_none_when_both_fail(self):
        """_resolve_nastech_bin returns None when both strategies fail."""
        from gateway.run import _resolve_nastech_bin
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((nastech_home / ".update_pending.json").read_text())
...
class TestSendUpdateNotification:
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (nastech_home / ".update_pending.json").write_text(json.dumps(pending))
        (nastech_home / ".update_output.txt").write_text("x" * 5000)
        (nastech_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (nastech_home / ".update_pending.json").write_text(json.dumps(pending))
        (nastech_home / ".update_output.txt").write_text("Traceback: boom")
