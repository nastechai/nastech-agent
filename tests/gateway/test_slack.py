...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@nastech`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@nastech"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@nastech")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /nastech btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_nastech_freeform_question(self, adapter):
        """/nastech <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/nastech",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_nastech_slash_stashes_context(self, adapter):
        """Legacy /nastech <subcommand> also stashes context."""
        command = {
            "command": "/nastech",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_nastech_question_does_not_stash_context(self, adapter):
        """Free-form /nastech <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/nastech",
