...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_nastech_variants(self, monkeypatch):
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Nastech what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Nastech, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Nastech agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Nastech about this") == "please ask Nastech about this"
