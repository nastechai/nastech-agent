...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 nastech-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["nastech-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * nastech-bot :Nickname is already in use")
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 nastech :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * nastech :Nickname in use")
        assert adapter._current_nick == "nastech_"
        sent = writer.write.call_args[0][0]
        assert b"NICK nastech_" in sent
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG nastech :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":nastech!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :nastech: hello")
        assert len(dispatched) == 1
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * nastech :Nickname in use")
        assert adapter._current_nick == "nastech_"
        await adapter._handle_line(":server 433 * nastech_ :Nickname in use")
        assert adapter._current_nick == "nastech_1"
        await adapter._handle_line(":server 433 * nastech_1 :Nickname in use")
        assert adapter._current_nick == "nastech_2"
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "nastechbot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 nastechbot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK nastechbot-cron") for line in sent_lines)
        assert any(line.startswith("USER nastechbot-cron 0 * :Nastech Agent (cron)")
    ...
    async def test_standalone_send_strips_crlf_from_message_body(self, monkeypatch):
        ...
        assert all(line.startswith("NICK nastechbot-cron") for line in nick_lines)
    ...
    async def test_standalone_send_joins_channel_before_privmsg(self, monkeypatch):
        ...
        conn = _FakeIRCConnection([
            b":server 001 nastechbot-cron :Welcome",
            b":server 366 nastechbot-cron #cron :End of /NAMES list.",
        ])
