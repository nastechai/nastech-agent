...
def test_env_override_beats_everything(isolated_home, monkeypatch):
    """NASTECH_MAX_TOKENS is the internal override mechanism (highest priority)."""
    write_cfg, fresh_gateway = isolated_home
    monkeypatch.setenv("NASTECH_MAX_TOKENS", "2048")
    write_cfg(
        """
        model:
          default: glm-5.1
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("nastech_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("nastech_cli.runtime_provider")
