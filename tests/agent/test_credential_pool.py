...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    assert {entry.source for entry in entries} == {"manual:nastech_pkce", "nastech_pkce"}
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `nastech auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `nastech model` to reauth, but the pool
    ...
    # Simulate `nastech model` / `nastech auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from nastech_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
...
def test_is_terminal_codex_oauth_refresh_error():
    from nastech_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
