...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``nastech_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = nastech_state.SessionDB
    monkeypatch.setattr(nastech_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(nastech_state, "SessionDB", real_session_db_type)
