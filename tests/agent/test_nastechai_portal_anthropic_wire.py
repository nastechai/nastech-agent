...
``https://inference-api.nastechai.com/v1/messages`` alongside the
...
PORTAL_URL = "https://inference-api.nastechai.com/v1"
# Staging / preview hosts used via nastechai_INFERENCE_BASE_URL — not the prod
# hostname, so Portal behaviour must key off provider=nastechai.
...
class TestApiModeRouting:
    """``nastechai_api_mode`` / ``determine_api_mode`` split the Portal catalog."""
    ...
    def test_anthropic_prefixed_models_use_the_messages_wire(self, model):
        assert nastechai_api_mode(model) == "anthropic_messages"
    ...
    def test_a_claude_model_without_the_vendor_prefix_is_not_rerouted(self):
        ...
        assert nastechai_api_mode("claude-opus-4.8") == "chat_completions"

    def test_determine_api_mode_honors_the_model_for_nastechai(self):
        ...
        overlay alone advertises openai_chat for every nastechai model."""
        ...
        assert (
            determine_api_mode("nastechai", PORTAL_URL, model="nastech-4-405b")
            == "chat_completions"
        )
        # No model → historical OpenAI-wire default (safer than guessing).
        assert determine_api_mode("nastechai", PORTAL_URL) == "chat_completions"
...
class TestRuntimeResolution:
    ...
    def test_anthropic_model_resolves_to_the_messages_wire(self, monkeypatch):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="nastechai", target_model="anthropic/claude-opus-5"
        )
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "nastechai", "default": "nastech-4-405b"},
        )
...
class TestPoolRuntimeResolution:
    ...
    def test_pool_entry_honors_the_model_derived_mode(self, monkeypatch, portal_entry):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="nastechai", target_model="anthropic/claude-opus-4.8"
        )
...
class TestClientShape:
    ...
    def test_lookalike_host_does_not_get_portal_treatment(self):
        ...
        from agent.anthropic_adapter import (
            _is_nastechai_portal_endpoint,
            _requires_bearer_auth,
        )

        spoofed = "https://inference-api.nastechai.com.attacker.test/v1"
        assert not _is_nastechai_portal_endpoint(spoofed)
...
class TestPortalBodyFields:
    ...
    They are produced by the nastechai provider profile, which only the OpenAI-wire
    transport consults — so the Anthropic branch has to merge them in itself.
    """

    def _build(self, provider="nastechai", session_id="sess-abc123"):
    ...
    def test_helper_merge_is_a_no_op_for_non_nastechai(self):
        from agent.chat_completion_helpers import (
            _merge_nastechai_portal_messages_extra_body,
        )

        kwargs = {"model": "claude-opus-4-8", "messages": []}
        agent = SimpleNamespace(provider="anthropic", session_id="s")
        assert _merge_nastechai_portal_messages_extra_body(agent, kwargs) is kwargs
...
class TestPortalThinkingReplay:
    ...
    def test_staging_host_with_env_override_keeps_signed_thinking(
        ...
        monkeypatch.setenv("nastechai_INFERENCE_BASE_URL", STAGING_URL)
...
class TestAuxiliaryDualWire:
    """``resolve_provider_client('nastechai', …)`` must wrap Claude onto Messages."""
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                "agent.auxiliary_client._try_nastechai",
            ...
            client, model = resolve_provider_client("nastechai", "nastech-4-405b")
