...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_nastech_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / ".nastech"))

    nastech_env = tmp_path / ".nastech" / ".env"
    nastech_env.parent.mkdir(parents=True)
    nastech_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.nastech/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
