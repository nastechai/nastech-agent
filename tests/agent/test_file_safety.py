...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_nastech_env(self):
        """Nastech' own .env inside NASTECH_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: nastech internal .env is in ~/.nastech/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.nastech/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.nastech/.env"))
        assert error is not None
