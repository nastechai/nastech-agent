...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_nastech, monkeypatch):
        _set_active_home(monkeypatch, fake_nastech["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "nastech-security"
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_nastech, monkeypatch):
        _set_active_home(monkeypatch, fake_nastech["security_home"])
    ...
    def test_named_to_named_cross_profile(self, fake_nastech, monkeypatch):
        _set_active_home(monkeypatch, fake_nastech["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_nastech["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
    ...
    def test_non_nastech_path_returns_none(self, fake_nastech, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_nastech["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Nastech root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_nastech_config_not_classified_as_cross_profile(self, fake_nastech, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_nastech["default_home"] / "config.yaml")
        )
        assert result is None
