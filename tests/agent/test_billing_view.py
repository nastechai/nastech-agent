...
def test_portal_base_url_env_override(monkeypatch):
    monkeypatch.setenv("NASTECH_PORTAL_BASE_URL", "https://preview.example.com/")
...
def test_billing_fixture_unset_returns_none(monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_DEV_BILLING_FIXTURE", raising=False)
...
def test_billing_fixture_autoreload_state(monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_DEV_BILLING_FIXTURE", "card-autoreload")
    s = build_billing_state()
...
def test_billing_fixture_logged_out_and_unknown(monkeypatch):
    monkeypatch.setenv("NASTECH_DEV_BILLING_FIXTURE", "logged-out")
    assert build_billing_state().logged_in is False
    monkeypatch.setenv("NASTECH_DEV_BILLING_FIXTURE", "bogus-state")
    s = build_billing_state()
