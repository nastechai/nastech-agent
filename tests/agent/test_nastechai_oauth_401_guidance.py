...
def test_nastechai_provider_is_in_oauth_401_set():
    """The provider-set gate that selects OAuth-specific guidance must
    include ``nastechai`` alongside ``openai-codex`` and ``xai-oauth``.
    """
    ...
    assert "\"nastechai\"" in source

    # And the gate string itself must mention all three so future refactors
    # that split nastechai off into its own gate still get caught.
    needle = "_provider in {\"openai-codex\", \"xai-oauth\", \"nastechai\"}"
    assert needle in source, (
        "Expected nastechai to be co-gated with the other OAuth providers in the "
