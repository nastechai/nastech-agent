...
class TestGetDefaultHermesRoot:
    ...
    def test_nastech_home_is_native(self, tmp_path, monkeypatch):
        """When NASTECH_HOME = ~/.nastech, returns ~/.nastech."""
        native = tmp_path / ".nastech"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_HOME", str(native))
        assert get_default_nastech_root() == native

    def test_nastech_home_is_profile(self, tmp_path, monkeypatch):
        """When NASTECH_HOME is a profile under ~/.nastech, returns ~/.nastech."""
        native = tmp_path / ".nastech"
    ...
    def test_nastech_home_is_docker(self, tmp_path, monkeypatch):
        """When NASTECH_HOME points outside ~/.nastech (Docker), returns NASTECH_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_HOME", str(docker_home))
        assert get_default_nastech_root() == docker_home

    def test_nastech_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any NASTECH_HOME outside ~/.nastech is treated as the root."""
        custom = tmp_path / "my-nastech-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_HOME", str(custom))
        assert get_default_nastech_root() == custom
    ...
    def test_no_nastech_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/nastech without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("NASTECH_HOME", raising=False)
        ...
        assert get_default_nastech_root() == home / "AppData" / "Local" / "nastech"
...
class TestGetProcessHermesHome:
    ...
    def test_env_unset_returns_platform_default(self, tmp_path, monkeypatch):
        monkeypatch.delenv("NASTECH_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        assert get_process_nastech_home() == tmp_path / ".nastech"
    ...
    def test_ignores_context_local_override(self, tmp_path, monkeypatch):
        ...
        try:
            # get_nastech_home() follows the override; the process-scoped
            # variant must not.
            assert get_nastech_home() == profile_home
            assert get_process_nastech_home() == launch_home
        finally:
...
class TestHermesManagedNode:
    ...
    def test_with_nastech_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "nastech"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_nastech_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_nastech_node_executable("npm") == str(healed_npm)
...
class TestReasoningOverridesDefaultConfig:
    ...
    def test_load_config_preserves_user_reasoning_overrides(self, tmp_path, monkeypatch):
        ...
        import yaml
        from nastech_cli.config import load_config, get_config_path
...
class TestGetHermesDir:
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_nastech_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_nastech_dir("cache/audio", "audio_cache")
...
class TestWslPathTranslation:
    ...
    def test_wsl_unc_to_posix_both_spellings(self):
        assert nastech_constants.wsl_unc_path_to_posix(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        assert nastech_constants.wsl_unc_path_to_posix(r"\\wsl$\Ubuntu\home\alex") == "/home/alex"
        # Forward-slash spelling and distro root.
        assert nastech_constants.wsl_unc_path_to_posix("//wsl.localhost/Debian/srv/app") == "/srv/app"
        assert nastech_constants.wsl_unc_path_to_posix("\\\\wsl.localhost\\Ubuntu\\") == "/"

    def test_wsl_unc_ignores_non_unc_paths(self):
        assert nastech_constants.wsl_unc_path_to_posix(r"C:\Users\alex") is None
        assert nastech_constants.wsl_unc_path_to_posix("/home/alex") is None

    def test_translate_is_noop_off_wsl(self, monkeypatch):
        monkeypatch.setattr(nastech_constants, "is_wsl", lambda: False)
        assert nastech_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == r"C:\Users\alex"
