...
from acp_adapter.server import NastechACPAgent, NASTECH_VERSION
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "nastech-agent"
        assert resp.agent_info.version == NASTECH_VERSION
...
class TestSessionOps:
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "NastechACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("NastechACPAgent")
    ...
    async def test_load_session_replays_history_before_returning_response(self, agent):
        ...
        counts synchronastechaily against the load response — see #12285 follow-up.
        """
        new_resp = await agent.new_session(cwd="/tmp")
        state = agent.session_manager.get_session(new_resp.session_id)
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_nastech_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``NASTECH_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("NASTECH_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "NASTECH_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("NASTECH_SESSION_ID") is None, (
            "NASTECH_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_nastech_session_id(self, agent, monkeypatch):
        """If the env already had NASTECH_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("NASTECH_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("NASTECH_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("NASTECH_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert NASTECH_VERSION in result
