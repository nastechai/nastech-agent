"""Tests for ACP Registry metadata shipped with Nastech."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "nastech-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Nastech Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NastechaiResearch/nastech-agent"
    assert data["website"].startswith("https://nastech-agent.nastechairesearch.com/")
    assert data["authors"] == ["Nastechai Research"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"nastech-agent[acp]=={data['version']}"
    assert uvx["args"] == ["nastech-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"nastech-agent[acp]=={_pyproject_version()}"
    )
