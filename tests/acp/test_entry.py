...
def test_main_version_prints_without_starting_server(monkeypatch, capsys):
    ...
    assert output
    assert "Starting nastech-agent ACP adapter" not in output
...
def test_main_check_prints_ok_without_starting_server(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.strip() == "Nastech ACP check OK"


def test_main_setup_runs_model_configuration(monkeypatch):
    calls = {}

    def fake_nastech_main():
        calls["argv"] = sys.argv[:]

    monkeypatch.setattr("nastech_cli.main.main", fake_nastech_main)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`nastech-acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
