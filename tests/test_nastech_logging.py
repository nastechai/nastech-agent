    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("nastech_cli.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in nastech_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                nastech_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        with patch.object(nastech_logging.sys, "platform", "linux"):
            assert not nastech_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestAsyncQueueLogging:
    ...
    def test_records_reach_file_through_queue(self, nastech_home):
        nastech_logging.setup_logging(nastech_home=nastech_home)
    ...
    def test_queue_preserves_per_handler_levels(self, nastech_home):
        nastech_logging.setup_logging(nastech_home=nastech_home)
