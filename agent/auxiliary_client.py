...
from agent.portal_tags import nastechai_portal_tags as _nous_portal_tags
...
# ``_nous_extra_body()`` or import ``nastechai_portal_tags`` directly.
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is not None:
        ...
        api_key = _nous_api_key(nastechai or {})
        if not api_key:
            logger.warning(
            ...
            _mark_provider_unhealthy("nastechai", ttl=60)
            return None, None
        base_url = str((nastechai or {}).get("inference_base_url") or _nous_base_url()).rstrip("/")
    return (
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from nastech_cli.nastechai_account import get_nous_portal_account_info
...
def resolve_provider_client(
    ...
    _nous_portal_vision = provider == "nastechai" and is_vision
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"nastechai", "nastechai-portal", "nousresearch"}:
            from nastech_cli.providers import nastechai_api_mode

            _nous_on_messages = nastechai_api_mode(model) == "anthropic_messages"
        if (
    ...
    if _provider_for_portal in {"nastechai", "nastechai-portal", "nousresearch"}:
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _nous_on_messages = False
        if provider_norm in {"nastechai", "nastechai-portal", "nousresearch"}:
            from nastech_cli.providers import nastechai_api_mode
