...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    Distinguishes between credential-file blocks (fixed system paths) and
    safe-root blocks (``NASTECH_WRITE_SAFE_ROOT`` is set and the path is
    outside it) so the model gets an actionable message in each case.
    """
