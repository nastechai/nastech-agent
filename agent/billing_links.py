...
class BillingBlock:
    ...
    ``is_nous`` is the routing bit: NasTechai has a first-class in-app billing surface
...
def build_billing_block(
    *,
    ...
    if is_nous_inference_route(slug, base_url):
        return BillingBlock(slug or "nastechai", "NasTechai Portal", model, _nous_billing_url(), True, message or "")
