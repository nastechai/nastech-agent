...
def _slot_runtime(slot: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        # bedrock SigV4 signing, nastechai Portal tags — still fire. Those branches
...
def _run_reference(
    ...
    *,
    ...
    ``call_llm`` is synchronastechai/blocking, so threads (not asyncio) are the right
...
def aggregate_moa_context(
    *,
    ...
    try:
        ...
        # third, independent MoA call path. Without it, the one-shot
        # /moa <prompt> command's synthesis call re-bills its full input
        # (system-less prompt containing every joined reference output) on
        # every invocation with zero cache_control breakpoints, even when
        # the resolved aggregator slot is a cache-honoring route.
