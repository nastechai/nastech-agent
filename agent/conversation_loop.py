...
def _nastechai_entitlement_message(capability: str) -> str:
    try:
...
def _print_nastechai_entitlement_guidance(agent, capability: str) -> bool:
    message = _nastechai_entitlement_message(capability)
    if not message:
        return False
...
def _is_nastechai_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        ...
        or base_url_host_matches(base, "inference.nastechairesearch.com")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_nastechai_inference_route(provider, base_url):
        return _nastechai_entitlement_message(capability)
...
def _try_refresh_nastechai_paid_entitlement_credentials(agent) -> bool:
    """Refresh Nastechai runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_nastechai_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nastechai":
                try:
                    from agent.nastechai_rate_guard import (
                        nastechai_rate_limit_remaining,
                        format_remaining as _fmt_nastechai_remaining,
                    )
                    ...
                    if _nastechai_remaining is not None and _nastechai_remaining > 0:
                        ...
                        agent._buffer_vprint(
                            f"⏳ {_nastechai_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_nastechai_msg}")
            ...
            try:
                ...
                # Clear Nastechai rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Nastechai.
                if agent.provider == "nastechai":
                    try:
                        from agent.nastechai_rate_guard import clear_nastechai_rate_limit
                        clear_nastechai_rate_limit()
                    except Exception:
                        pass
                ...
                if (
                    ...
                    and _is_nastechai_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.nastechai_paid_entitlement_refresh_attempted
                ):
                    _retry.nastechai_paid_entitlement_refresh_attempted = True
                    if _try_refresh_nastechai_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Nastechai paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "nastechai"
                    and status_code == 401
                    and not _retry.nastechai_auth_retry_attempted
                ):
                    _retry.nastechai_auth_retry_attempted = True
                    if agent._try_refresh_nastechai_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Nastechai agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Nastechai 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nastechai_entitlement_guidance(agent, "Nastechai model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: nastech auth add nastechai")
                ...
                # block every Nastechai model for minutes.  We use
                # ``is_genuine_nastechai_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_nastechai_rate_limit = False
                    try:
                        from agent.nastechai_rate_guard import (
                            is_genuine_nastechai_rate_limit,
                            record_nastechai_rate_limit,
                        )
                        ...
                        _genuine_nastechai_rate_limit = is_genuine_nastechai_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_nastechai_rate_limit:
                            record_nastechai_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "Nastechai 429 looks like upstream capacity "
                    ...
                    if _genuine_nastechai_rate_limit:
                        ...
                        # top-of-loop Nastechai guard handles fallback or
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "nastechai" and _print_nastechai_entitlement_guidance(
                            agent,
                            "Nastechai model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "nastechai"} and status_code == 401:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    from nastech_cli.plugins import get_pre_verify_continue_message, has_hook
