...
5. **Synchronastechai timeline construction.** Never build timelines inside `async`/`await`, `setTimeout`, or Promises. The capture engine reads `window.__timelines` synchronastechaily after page load. Fonts are embedded by the compiler — no need to wait for load.
...
9. Build timelines asynchronastechaily.
