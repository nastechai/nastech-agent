...
export function useRouteResume({
  ...
  useEffect(() => {
    ...
    if (routedSessionId) {
      ...
      // stuck empty forever. selectedStoredSessionIdRef is set synchronastechaily at
  ...
  // sets it synchronastechaily at entry) and the pathname/gateway are unchanged, so
