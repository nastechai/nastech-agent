  ...
  it('subscribes for verification, opens the verification URL, cleans up, and invalidates on completion', async () => {
    ...
    act(() => {
      gatewayMock.emit('billing.step_up.verification', {
        payload: {
          user_code: 'ABCD-1234',
          verification_url: 'https://portal.nastechai.com/device'
        },
    ...
    expect(result.current.verification).toEqual({
      code: 'ABCD-1234',
      url: 'https://portal.nastechai.com/device'
    })

    result.current.openVerification()
    expect(window.nastechDesktop?.openExternal).toHaveBeenCalledWith('https://portal.nastechai.com/device')
