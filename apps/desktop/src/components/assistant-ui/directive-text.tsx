...
const NASTECH_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
...
const ICON_PATHS: Record<NastechRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as NastechRefType] ?? ICON_FALLBACK
...
const NASTECH_DIRECTIVE_RE = new RegExp(
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(NASTECH_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as NastechRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: NastechRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
