...
export function getNastechConfig(): Promise<NastechConfig> {
...
export function selectToolsetProvider(
  ...
  return window.nastechDesktop.api<{ ok: boolean; name: string; provider: string }>({
    ...profileScoped(),
...
export interface RecommendedDefaultModel {
  provider: string
  model: string
  /** True/false for Nastechai (free vs paid tier); null for other providers. */
  free_tier: boolean | null
}

// Recommended default model for a freshly-authenticated provider. Mirrors the
// curation `nastech model` does — for Nastechai it honors the free/paid tier so a
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.nastechDesktop.api<DebugShareResponse>({
    ...
    method: 'POST',
    body: {},
    // Synchronastechai upload of report + logs to the paste service.
    timeoutMs: 120_000
  })
}
