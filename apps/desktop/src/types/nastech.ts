...
export interface ModelOptionProvider {
  ...
  /** Nastechai only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Nastechai only: paid models a free-tier user cannot select (shown disabled). */
...
/** One Nastechai-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Nastechai Tool Gateway as a result of
   *  switching the main provider to Nastechai. Empty unless provider === 'nastechai'
