...
test('locateNasTech canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v nastech/, '/home/u/.local/bin/nastech\n'],
    [/\[ -x .*\.local\/bin\/nastech/, 'OK'],
    [/python3 -c/, '/home/u/.nastech/nastech-agent/venv/bin/nastech\n']
  ])

  assert.equal(await locateNasTech(ssh, ''), '/home/u/.nastech/nastech-agent/venv/bin/nastech')
})
