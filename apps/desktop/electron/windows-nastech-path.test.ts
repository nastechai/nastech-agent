...
test('resolveVenvHermesCommand: returns null off Windows', () => {
  ...
  assert.equal(resolveVenvHermesCommand('/root/venv/Scripts/nastech.exe', [], deps), null)
})

test('resolveVenvHermesCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvHermesCommand('/root/venv/Scripts/nastech.cmd', [], deps), null)
})

test('resolveVenvHermesCommand: returns null when the basename is not nastech/nastech.exe', () => {
  const deps = makeDeps()
...
test('resolveVenvHermesCommand: returns null when the parent dir is not Scripts', () => {
  ...
  assert.equal(resolveVenvHermesCommand('/root/venv/bin/nastech.exe', [], deps), null)
})
...
test('resolveVenvHermesCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  ...
  const result = resolveVenvHermesCommand('/root/venv/Scripts/nastech.exe', ['serve'], deps)
...
test('resolveVenvHermesCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvHermesCommand('/root/venv/Scripts/nastech.exe', ['serve', '--port', '0'], deps)
...
test('resolveVenvHermesCommand: is case-insensitive on nastech.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvHermesCommand('/root/venv/Scripts/HERMES.EXE', [], deps))
  assert.ok(resolveVenvHermesCommand('/root/venv/SCRIPTS/nastech.exe', [], deps))
})
