 ...
 *      internals; NASTECH_HOME / NASTECH_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'nastech')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_NasTech12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/nastech/nastech', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/NasTech/nastech', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.nastech/nastech-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'nastech'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'nastech')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'nastech://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'nastech://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves NASTECH_HOME + NASTECH_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    NASTECH_HOME: '/home/u/.nastech',
    NASTECH_DESKTOP_REMOTE_URL: 'http://box:9119',
    NASTECH_DESKTOP_REMOTE_TOKEN: 'secret',
    NASTECH_DESKTOP_NASTECH_ROOT: '/home/u/dev/nastech',
    NASTECH_DESKTOP_APP_NAME: 'NasTechSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.nastech/nastech-agent/apps/desktop/release/linux-unpacked/NasTech',
    args: ['nastech://open/agent/42', "--note=it's fine"],
    env: { NASTECH_HOME: '/home/u/.nastech', NASTECH_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export NASTECH_HOME='\/home\/u\/\.nastech'/)
  assert.match(script, /export NASTECH_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/NasTech' 'nastech:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `nastech-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/NasTech/NasTech',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `nastech-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/NasTech\/NasTech'\n/)
})
