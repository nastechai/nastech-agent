...
function canImportHermesCli(pythonPath: string, opts: { env?: Record<string, string> } = {}) {
  ...
  try {
    execFileSync(pythonPath, ['-c', nastechRuntimeImportProbe()], {
 ...
 * @returns {boolean}
 */
function verifyHermesCli(nastechCommand: string, opts?: { shell?: boolean }) {
  if (!nastechCommand) {
    return false
  }
...
export { canImportHermesCli, nastechRuntimeImportProbe, PROBE_TIMEOUT_MS, verifyHermesCli }
