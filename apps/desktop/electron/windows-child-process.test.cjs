...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /nastechProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'nastech_cli\.main', 'uninstall', '--gui-summary'\]/)

  assert.match(source, /function unwrapWindowsVenvNastechCommand\(command, backendArgs\)/)
  assert.match(source, /function getVenvSitePackagesEntries\(venvRoot\)/)
  assert.match(source, /path\.join\(venvRoot, 'Lib', 'site-packages'\)/)
  assert.match(source, /args: \['-m', 'nastech_cli\.main', \.\.\.backendArgs\]/)
})
...
test('desktop backend launches console python so child consoles are inherited, not pythonw', () => {
  ...
  // NASTECH_DASHBOARD_READY stdout line — no ready-file side channel is set.
...
test('desktop backend teardown tree-kills Windows backend descendants', () => {
  ...
  const resetIndex = source.indexOf('function resetNastechConnection()')
  assert.notEqual(resetIndex, -1, 'missing resetNastechConnection')
  const resetSnippet = source.slice(resetIndex, resetIndex + 300)
  assert.match(resetSnippet, /stopBackendChild\(nastechProcess\)/)
  assert.doesNotMatch(resetSnippet, /nastechProcess\.kill\('SIGTERM'\)/)
  ...
  assert.match(quitSnippet, /stopBackendChild\(nastechProcess\)/)
  assert.doesNotMatch(quitSnippet, /nastechProcess\.kill\('SIGTERM'\)/)
})
