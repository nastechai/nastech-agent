 ...
 * Background
 * ----------
 * After `nastech update` + `nastech desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything NASTECH_DESKTOP_* is preserved
// plus NASTECH_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['NASTECH_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['NASTECH_DESKTOP_']
