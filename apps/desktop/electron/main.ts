...
function resolveHermesHome() {
  if (process.env.NASTECH_HOME) {
    return normalizeHermesHomeRoot(process.env.NASTECH_HOME)
  }
...
const NASTECH_HOME = resolveHermesHome()
...
// local backend as. When set, startHermes() passes `nastech --profile <name>
...
// electron-builder derives from the build `appId` (com.nastechai.nastech) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nastechai.nastech')
}
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    isHermesSourceRoot(ACTIVE_NASTECH_ROOT) ? ACTIVE_NASTECH_ROOT : null
  ].filter(Boolean)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'nastech.exe' : 'nastech')
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvHermes) || fileExists(path.join(updateRoot, '.nastech-bootstrap-complete'))
...
function resolveHermesCliBinary(updateRoot) {
  const venvHermes = path.join(updateRoot, 'venv', 'bin', 'nastech')
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const nastech = resolveHermesCliBinary(updateRoot)
...
function isActiveRuntimeUsable() {
  ...
  return (
    isHermesSourceRoot(ACTIVE_NASTECH_ROOT) &&
...
function resolveHermesBackend(backendArgs) {
  ...
  if (process.env.NASTECH_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (nastechCommand) {
      const unwrapped = unwrapWindowsVenvHermesCommand(nastechCommand, backendArgs)
      ...
      if (verifyHermesCli(nastechCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvHermesCommand(nastechCommand, backendArgs) || {
...
async function ensureRuntime(backend) {
  ...
  if (!isHermesSourceRoot(ACTIVE_NASTECH_ROOT)) {
    throw new Error(
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  // saved mode, preserving 'cloud' (a Nastech Cloud connection — Q6) so the UI
...
async function spawnPoolBackend(profile, entry) {
  ...
  const nastechCwd = resolveHermesCwd()
...
ipcMain.handle('nastech:connection-config:apply', async (_event, payload) => {
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on nastech:connection:applied.
