...
const { canImportNastechCli, verifyNastechCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeNastechHomeRoot } = require('./backend-env.cjs')
...
function resolveNastechHome() {
  if (process.env.NASTECH_HOME) return normalizeNastechHomeRoot(process.env.NASTECH_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'nastech-home')
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) return normalizeNastechHomeRoot(fromRegistry)
  }
...
function nastechManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_nastech_node_dirs() in
  // nastech_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(NASTECH_HOME, 'node')
...
function pathWithNastechManagedNode(...entries) {
  return [...nastechManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}
...
let mainWindow = null
let nastechProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by nastechProcess +
// connectionPromise + startNastech(); this pool only holds EXTRA profile
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(NASTECH_HOME)
  ...
  while (marker && Date.now() < deadline) {
    ...
    marker = readLiveUpdateMarker(NASTECH_HOME)
  }
  if (marker) {
...
function unwrapWindowsVenvNastechCommand(command, backendArgs) {
  ...
  if (!/^nastech(?:\.exe)?$/i.test(path.basename(resolved))) return null
...
// findGitBash — locate bash.exe on Windows. Nastech' terminal tool requires
...
function findGitBash() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nastech', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'nastech', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// NASTECH_HOME/nastech-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_nastech_home). That binary owns
// ALL repo mutation — running `nastech update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'nastech-setup.exe' : 'nastech-setup'
  const candidate = path.join(NASTECH_HOME, name)
...
// the whole tree synchronastechaily. Windows-only: this is called solely from the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (nastechProcess && Number.isInteger(nastechProcess.pid)) pids.push(nastechProcess.pid)
  ...
  if (nastechProcess && !nastechProcess.killed) {
    try {
      nastechProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (nastechProcess && Number.isInteger(nastechProcess.pid)) stragglers.push(nastechProcess.pid)
    for (const entry of backendPool.values()) {
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri nastech-setup. Unlike Windows
      ...
      // whole update itself: `nastech update` (backend) + `nastech desktop
    ...
    if (!updater) {
      ...
      // nastech-setup.exe into NASTECH_HOME). They DO have a working `nastech`
      ...
      // checkout they're on — bare `nastech update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `nastech update --branch ${branch}`
        }
      } catch {
...
function resolveNastechCliBinary(updateRoot) {
  ...
  if (fileExists(venvNastech)) return venvNastech
...
function resolveNastechBackend(backendArgs) {
  ...
  // 3. Bootstrap-complete ACTIVE_NASTECH_ROOT -- the canonical install at
  ...
  //    to spawning nastech. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `nastech update` from the CLI.
  ...
  if (process.env.NASTECH_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (nastechCommand) {
      ...
      if (verifyNastechCli(nastechCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvNastechCommand(nastechCommand, backendArgs) || {
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nastech install was cancelled.')
  ...
  if (!fileExists(venvPython)) {
    ...
    // isNastechSourceRoot() and the bootstrap writes the marker only after
...
async function waitForNastech(baseUrl, token) {
  ...
  throw new Error(`Nastech backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
const ZOOM_STORAGE_KEY = 'nastech:desktop:zoomLevel'
...
function installMediaPermissions() {
  ...
  // Synchronastechai check handler: Chromium consults this for getUserMedia on
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Nastech gateway',
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (nastech #37247), and the
...
function resetNastechConnection() {
...
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetNastechConnection() nulls nastechProcess.
...
// profile to startNastech() (the window backend: boot UI, bootstrap, remote
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForNastech(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
...
async function startNastech() {
  ...
  connectionPromise = (async () => {
    ...
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null nastechProcess when the child dies.
      childAlive: () => nastechProcess !== null && nastechProcess.exitCode === null && !nastechProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
...
function createWindow() {
  ...
  // the cross-platform backstop, flushed synchronastechaily before the window is gone.
...
ipcMain.handle('nastech:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetNastechConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Nastech backend failed liveness probe; dropping stale connection.')
    resetNastechConnection()
...
ipcMain.handle('nastech:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
...
ipcMain.handle('nastech:window:openNewSession', async () => {
...
ipcMain.handle('nastech:bootstrap:repair', async () => {
  ...
  // startNastech() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('nastech:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('nastech:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Nastech.')
  }
