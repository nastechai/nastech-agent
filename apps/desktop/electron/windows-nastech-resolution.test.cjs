...
// Regression guards for Windows `nastech` resolution in main.cjs.
//
...
//      Git-Bash `nastech` shim shadowed the real nastech.cmd/nastech.exe; the
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow nastech.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\nastech.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvNastech\) \?/,
    'recovery regressed to gating only on the nastech.exe shim, which forces destructive --repair'
  )
})
