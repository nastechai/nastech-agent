...
### "Reset permissions" / "make NasTech ask again"
...
1. Shell-command allowlist: `nastech config set command_allowlist '[]'`
2. Shell-hook consent (only if present): `rm -f ~/.nastech/shell-hooks-allowlist.json`

Then sanity-check `nastech config get approvals.mode` (should not be `off`)
...
To keep the model away from network or media tools entirely, open `nastech tools` and toggle per-platform. Takes effect on next session (`/reset`). See `references/configuration.md` for the toolset list.
