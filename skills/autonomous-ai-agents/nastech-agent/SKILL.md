...
- **Desktop app** (`nastech desktop` / `nastech gui`) — native Electron app
...
- **Web dashboard** (`nastech dashboard`) — full admin panel: configure
  ...
  an embedded `nastech --tui` chat. Secured behind an OAuth/token gate.
- **OpenAI-compatible proxy** (`nastech proxy`) — exposes a
...
- **Automation Blueprints** — pick a named automation and Nastech asks for
