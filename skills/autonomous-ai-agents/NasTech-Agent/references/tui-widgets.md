...
Author widget apps for the NasTech TUI (`nastech --tui`): glanceable ambient
...
- The TUI must be in use (`nastech --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.nastech/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.nastech/logs/tui_gateway_crash.log` if a widget never appears.
