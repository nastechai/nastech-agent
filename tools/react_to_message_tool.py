...
Gated on ``NASTECH_DESKTOP`` (like the other GUI affordances) so it costs nothing
...
def check_react_requirements() -> bool:
    ...
    NASTECH_DESKTOP is set on the gateway the app spawns; the feature itself is
    off by default and enabled from Settings → Appearance (the desktop mirrors
    the toggle into ``display.message_reactions``).
    """
    if not env_var_enabled("NASTECH_DESKTOP"):
        return False
    try:
