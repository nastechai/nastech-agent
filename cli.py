...
def _replay_output_history() -> None:
    ...
    try:
        ...
        if rendered_lines:
            ...
            # per-line prompt_toolkit print forces one synchronastechai terminal I/O
...
def _cprint(text: str):
    ...
    def _schedule():
        ...
        #     function directly since PT already executed it synchronastechaily.
        ...
        try:
            ...
            # else: run_in_terminal ran the lambda synchronastechaily; nothing more
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NASTECHAI NASTECH - AI Agent Framework"
        tiny_line = "⚕ NASTECHAI NASTECH"
    else:
...
class NastechCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            provider: Inference provider ("auto", "openrouter", "nastechai", "openai-codex", "zai", "kimi-coding", "minimax", "minimax-cn")
    ...
    def show_banner(self):
        ...
        # Warn if the configured model is a Nastechai Nastech LLM (not agentic)
        from nastech_cli.model_switch import is_nastechai_nastech_non_agentic
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        # Copilot, and Nastechai-enforced caps win over the raw models.dev entry
    ...
    def _show_usage(self):
        """Rate limits + session token usage (when a live agent exists) + Nastechai credits.

        The Nastechai credits block is agent-independent (a portal fetch), so it runs even
        ...
        """
        if not self.agent:
            if not self._print_nastechai_credits_block():
                print("(._.) No active agent -- send a message first.")
            return
        ...
        # Nastechai credits magnitudes + monthly-grant gauge (agent-independent — also
        # runs at the no-agent / no-calls early-returns above). See the helper.
        self._print_nastechai_credits_block()
    ...
    def _print_nastechai_credits_block(self) -> bool:
        """Print the Nastechai credits magnitudes + monthly-grant gauge when a Nastechai account
        is logged in. Returns True if it printed anything.

        Delegates to the shared ``agent.account_usage.nastechai_credits_lines`` helper —
        the single source for the /usage credits block across CLI, gateway, and TUI.
        It's agent-independent (a portal fetch gated on "a Nastechai account is logged in",
        ...
        wall-clock-bounded inside the helper; also honors NASTECH_DEV_CREDITS_FIXTURE
        for offline testing — same behavior as every other surface.
        """
        from agent.account_usage import nastechai_credits_lines

        lines = nastechai_credits_lines()
        if not lines:
            return False
        print()
        for line in lines:
    ...
    def _show_credits(self):
        """`/credits` — focused Nastechai credit balance + top-up handoff.
        ...
        if not view.logged_in:
            print()
            _cprint(f"  💳 {_d('Not logged into Nastechai Portal.')}")
            print("  Run `nastech portal` to log in, then /credits.")
            return

        print()
        print("  💳 Nastechai credits")
        print(f"  {'─' * 41}")
        for line in view.balance_lines:
            # Drop the helper's own "📈 Nastechai credits" header — we print our own.
    ...
    def _show_billing(self, command: str = "/billing"):
        """`/billing` — terminal billing for Nastechai (one interactive modal).
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nastechai Portal.')}")
                print("  Run `nastech portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Nastechai Research to charge your card."
        )
        ...
        from nastech_cli.nastechai_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from nastech_cli.nastechai_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `nastech portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from nastech_cli.auth import step_up_nastechai_billing_scope

            granted = step_up_nastechai_billing_scope(open_browser=True)
        except Exception as exc:
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Nastechai Research to charge {card.masked} "
