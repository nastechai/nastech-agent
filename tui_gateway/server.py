...
def _persist_session_git_meta(session: dict, cwd: str) -> None:
    ...
    Best-effort: ``cwd`` itself is persisted synchronastechaily by the caller, so a
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over NASTECH_MODEL / NASTECH_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set NASTECH_INFERENCE_MODEL in the
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _NASTECH_CORE_TOOLS (every other
...
# A delegated child is not a live gateway session — it runs synchronastechaily
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from nastech_cli.config import load_config as _load_full_cfg
...
def _resolve_runtime_with_fallback(
    ...
    ``nastech_cli/cli_agent_setup_mixin.py``: when the primary provider
...
def _(rid, params: dict) -> dict:
    ...
    # synchronastechaily (e.g. tests of the build race) passes ``eager_build: true``
...
def _(rid, params: dict) -> dict:
    ...
    # Nastechai credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into Nastechai
    # or on any portal hiccup.
    try:
        from agent.account_usage import nastechai_credits_lines

        credits = nastechai_credits_lines()
        if credits:
...
@method("credits.view")
def _(rid, params: dict) -> dict:
    """Structured Nastechai credit view for the TUI /credits command.

    Account-independent (a portal fetch gated on "a Nastechai account is logged in"),
...
# (agent/billing_view.py + nastech_cli/nastechai_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from nastech_cli.nastechai_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from nastech_cli.nastechai_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from nastech_cli.nastechai_billing import BillingError, patch_auto_top_up
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".nastech" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.nastech/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.nastech/desktop-attachments/``.
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from nastech_cli.model_switch import parse_model_flags
...
def _is_repo_junk(root: str) -> bool:
    ...
    return real == home or real == nastech_home or real.startswith(nastech_home + os.sep)
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from nastech_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("NASTECH_API_KEY", "") or cfg.get("api_key", "")
