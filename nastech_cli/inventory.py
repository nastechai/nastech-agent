...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (nastechai /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for nastechai, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``nastech model`` CLI picker. Adds network calls
      (pricing fetch + nastechai tier check); only set for interactive pickers.
    ...
    - ``force_fresh_nastechai_tier``: bypass the short nastechai free-tier cache when
      selecting Portal-recommended nastechai models and applying tier gating. Keep
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_nastechai_tier=force_fresh_nastechai_tier,
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_nastechai_tier=force_fresh_nastechai_tier)
    if capabilities:
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (nastechai, openrouter) serve dozens of models across many
...
def _apply_pricing(
    ...
    *,
    ...
    """Enrich each provider row with per-model pricing + nastechai tier gating.

    Mutates ``rows`` in-place. For every row whose provider supports live
    pricing (openrouter / nastechai / novita) adds::

        row["pricing"] = {model_id: {"input": "$3.00", "output": "$15.00",
                                     "cache": "$0.30" | None, "free": bool}}

    For nastechai additionally adds::
    ...
    # Resolve nastechai free-tier once (cached in models.py for the TTL window).
    nastechai_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            if slug == "nastechai" and not is_free:
        ...
        if slug == "nastechai":
            try:
                if nastechai_free_tier is None:
                    nastechai_free_tier = check_nastechai_free_tier(
                        force_fresh=force_fresh_nastechai_tier
                    )
                row["free_tier"] = bool(nastechai_free_tier)
                if nastechai_free_tier:
                    _selectable, unavailable = partition_nastechai_models_by_tier(
