"""Nastechai Portal terminal-billing HTTP client (Phase 2b).
...
- **Auth** = the OAuth bearer JWT Nastech already holds for inference
  (``get_provider_auth_state("nastechai")["access_token"]``). No API-key auth on these.
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``NASTECH_PORTAL_BASE_URL`` → ``NASTECHAI_PORTAL_BASE_URL`` → the
...
# nastech_cli.auth.NASTECHAI_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``NASTECH_PORTAL_BASE_URL`` → ``NASTECHAI_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("NASTECH_PORTAL_BASE_URL") or os.getenv("NASTECHAI_PORTAL_BASE_URL")
...
# Short-lived cache for the resolved (token, base). `resolve_nastechai_access_token`
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    (``resolve_nastechai_access_token``), so a short-lived (~15 min) access token that
    ...
    when there is no usable Nastechai session at all.
    ...
    try:
        token = resolve_nastechai_access_token()
