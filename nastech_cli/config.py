...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONNASTECHAIERSITE",
    # Node
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "NASTECH_TOOL_PROGRESS", "NASTECH_TOOL_PROGRESS_MODE",
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/nastech`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$NASTECH_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    3. NASTECH_MANAGED env / .managed marker (NixOS, Homebrew)
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # blocks indefinitely (input() is synchronastechai) and ignores this.
    ...
    # See tools/kanban_tools.py and nastech_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``nastech dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        "transient_retries": 2,
        "vision": {
            "provider": "auto",    # auto | openrouter | nastechai | codex | custom
    ...
    "display": {
        ...
        # Nastechai credits status-bar notices (usage bands, grant-spent, depleted /
    ...
    "dashboard": {
        ...
        # nastechai-account-service at deploy time via the
    ...
    # configured providers (OpenRouter, Nastechai, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        "max_concurrent_children": 3,  # unified concurrency cap: max parallel children per batch
                                       ...
                                       # fall back to synchronastechai execution. Floor of 1, no ceiling.
    ...
    "updates": {
        ...
        # zipping a large NASTECH_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``nastech update
...
OPTIONAL_ENV_VARS = {
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Nastechai Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Nastechai Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nastechai Subscriber access token for tool-gateway requests (optional; otherwise read from the Nastech auth store)",
...
def require_readable_config_before_write(config_path: Optional[Path] = None) -> None:
    ...
    Guards every write path so a full-file replacement can never silently
    clobber an existing config.yaml that degraded to an empty dict on read
    (permission error, broken mount, transient I/O). New-file creation still
    works when the path is absent.
    """
