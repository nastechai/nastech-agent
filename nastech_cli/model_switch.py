...
# ---------------------------------------------------------------------------
def is_nastechai_nastech_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real nastechai Nastech 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_nastechai_NASTECH_NON_AGENTIC_RE.search(model_name))


def _check_nastech_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a nastechai Nastech 3/4 chat model."""
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "nastechai")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "nastechai")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, nastechai, and falls back to models.dev for the
    rest.
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, nastechai, Codex, GMI, Ollama, models.dev and
...
def switch_model(
    ...
    if target_provider in {"nastechai", "nastechai-portal", "nastechairesearch"}:
        from nastech_cli.providers import nastechai_api_mode

        api_mode = nastechai_api_mode(new_model)
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_nastechai_tier: bool = False,
    ...
    ``force_fresh_nastechai_tier`` bypasses the short nastechai tier cache for explicit
    ...
    from nastech_cli.models import (
        ...
        clear_provider_models_cache, get_curated_nastechai_model_ids,
    )
    ...
    curated["nastechai"] = get_curated_nastechai_model_ids()
    ...
    # --- 2. Check Nastech-only providers (nastechai, openai-codex, copilot, opencode-go) ---
    ...
    for pid, overlay in NASTECH_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        ...
        elif nastech_slug == "nastechai":
            # nastechai serves a large live /v1/models catalog (vendor-prefixed
            ...
            # order. Mirror that exactly (see _model_flow_nastechai in main.py) so
            ...
            model_ids = curated.get("nastechai", [])
            try:
                from nastech_cli.models import (
                    get_pricing_for_provider as _nastechai_pricing,
                    check_nastechai_free_tier as _nastechai_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from nastech_cli.auth import get_provider_auth_state as _nastechai_state

                _pricing = _nastechai_pricing("nastechai") or {}
                _portal = ""
                try:
                    _st = _nastechai_state("nastechai") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _nastechai_free(force_fresh=force_fresh_nastechai_tier):
