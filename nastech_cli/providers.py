...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Nastech overlays (for providers not in models.dev: nastechai, openai-codex, etc.)
...
def nastechai_api_mode(model: str = "") -> str:
    ...
    historical nastechai transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (nastechai) whose
    ...
    # nastechai is dual-wire: anthropic/* → Messages, everything else →
    ...
    if provider_norm in {"nastechai", "nastechai-portal", "nastechairesearch"}:
        return nastechai_api_mode(model)
