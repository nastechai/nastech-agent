...
def cmd_chat(args):
    ...
    # setup (config, rules files, plugins, MCP servers) or from Nastech itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (NASTECH_SAFE_MODE is checked by
    # nastech_cli/plugins.py and tools/mcp_tool.py).
...
def select_provider_and_model(args=None):
    ...
    elif selected_provider == "nastechai":
        _model_flow_nastechai(config, current_model, args=args)
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # Show update status (synchronastechai — acceptable since user asked for version info)
    try:
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/NastechaiResearch/nastech-agent/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NastechaiResearch/nastech-agent.git",
...
OFFICIAL_REPO_URL = "https://github.com/NastechaiResearch/nastech-agent.git"
    ...
    if not has_upstream:
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NastechaiResearch/nastech-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NastechaiResearch/nastech-agent.git' to add later."
            )
...
# Install-scoped breadcrumb dropped right before ``nastech update`` mutates the
...
# survives and the next ``nastech`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $NASTECH_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``nastech update`` left half-done.
    ...
    # Windows self-lock guard: if nastech.exe is the launcher that spawned
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Nastech is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Nastech from a different terminal, "
...
def _run_pre_update_backup(args) -> None:
    ...
    # ``nastech_cli/config.py``). Fall back to false if the key is missing
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (
            PROJECT_ROOT / ".nastech-bootstrap-complete",
            _update_marker_path(),
        )
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the marker survives and the next ``nastech`` launch finishes the
...
def cmd_dashboard(args):
    ...
    # (e.g. plugins/dashboard_auth/nastechai) registers BEFORE start_server's
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        ...
        return [
            "auto", "openrouter", "nastechai", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
def _prepare_agent_startup(args) -> None:
    ...
    if _run_inline_mcp_discovery:
        try:
            # MCP tool discovery remains synchronastechai for entrypoints that do
...
def main():
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nastech-agent.nastechairesearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from nastech_cli.tools_config import _cua_driver_cmd
            # Honor NASTECH_CUA_DRIVER_CMD for local-build testing — same
