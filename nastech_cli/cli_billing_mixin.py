...
This module hosts the nastechai billing/subscription methods lifted out of
...
class CLIBillingMixin:
    ...
    def _print_nastechai_credits_block(self) -> bool:
        """Print the nastechai dollar balance block (two-bar view) when a nastechai account
        ...
        truth). Falls back to the legacy ``nastechai_credits_lines`` text only when the
        model is unavailable. Agent-independent (a portal fetch gated on "a nastechai
    ...
    def _print_usage_cta(self) -> None:
        ...
        usage read with the same nudge. Only called when a nastechai account is logged
        in (the balance block printed), since both commands are nastechai-account only.
        """
    ...
    def _show_subscription(self):
        """`/subscription` (alias `/upgrade`) — view the nastechai plan + browser hand-off.
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from nastech_cli.nastechai_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        step_up_nastechai_billing_scope directly (it opens the browser + blocks), then
        ...
        try:
            from nastech_cli import nastechai_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        """`/topup` — Remote Spending for nastechai (one interactive modal).
