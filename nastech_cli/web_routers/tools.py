    ...
    def _read():
        with _profile_scope(profile):
            ...
            if cat:
                ...
                # re-probing per row.
                features = get_nastechai_subscription_features(config, force_fresh=True)
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        # including logged-out Nastechai Subscription rows and never-run
...
async def select_toolset_provider(
    ...
    Managed Nastechai rows (``managed_nastechai_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_nastechai_portal_access`` (inline login), but the GUI has no inline
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Nastechai rows — mirrors the gate the CLI
            # applies via ensure_nastechai_portal_access at selection time. This hits
            ...
            managed_feature = (row or {}).get("managed_nastechai_feature")
            if managed_feature:
                features = get_nastechai_subscription_features(config, force_fresh=True)
                ...
                if not entitled:
                    response["needs_nastechai_auth"] = True
