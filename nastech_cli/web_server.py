...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Nastechai, OpenAI Codex, or MiniMax).
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Nastechai, OpenAI Codex, MiniMax) and the
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # NASTECH_HOME override (see mcp_tool._wrap_with_home_override), so
