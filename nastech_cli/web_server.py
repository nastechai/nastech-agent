...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # nastechai bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose nastechai auth dies terminally (invalid_grant / quarantine)
        ...
        nastechai_session_valid = "unknown"
        try:
            from nastech_cli.auth import get_nastechai_session_validity
            nastechai_session_valid = get_nastechai_session_validity()
        except Exception:
            nastechai_session_valid = "unknown"
        ...
        status = {
            ...
            "nastechai_session_valid": nastechai_session_valid,
        }
...
def _get_portal_status_sync():
    ...
    try:
        ...
        feats = get_nastechai_subscription_features(cfg)
...
def get_recommended_default_model(provider: str = ""):
    ...
    nastechai this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for nastechai and None otherwise. `model` may be
    ...
    if slug == "nastechai":
        try:
            ...
            pricing = get_pricing_for_provider("nastechai") or {}
            free_tier = check_nastechai_free_tier(force_fresh=True)

            portal_url = ""
            try:
                state = get_provider_auth_state("nastechai") or {}
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_nastechai_models_by_tier(
            ...
            model = pick_silent_default_model(model_ids, provider="nastechai")
            return {"provider": "nastechai", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (nastechai) failed")
            return {"provider": "nastechai", "model": "", "free_tier": None}

    # Non-nastechai: preferred silent default when the provider's curated list
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # When switching the main provider to nastechai, mirror the CLI's
        # post-model-selection behaviour (nastech_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_nastechai_managed_defaults):
        # auto-route any *unconfigured* tools through the nastechai Tool Gateway.
        # This is purely additive — apply_nastechai_managed_defaults skips every
        ...
        if provider.strip().lower() == "nastechai":
            try:
                from nastech_cli.nastechai_subscription import apply_nastechai_managed_defaults
                from nastech_cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_nastechai_managed_defaults(
                    cfg,
                ...
                # Portal lookup hiccups / non-subscriber / non-nastechai gating
                # must never block saving the model assignment.
                _log.debug("apply_nastechai_managed_defaults skipped", exc_info=True)
        ...
        # a now-unpaid provider (e.g. nastechai with $0 balance) keeps paying 402s
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "google_chat": {
        ...
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/weixin/",
    ...
    "teams": {
        ...
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        ...
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Nastech from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Nastech over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Nastech through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Nastech over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Nastech via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
...
# Anthropic, device-code for nastechai/Codex) still runs in the CLI for now;
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "status_fn": None,  # dispatched via auth.get_nastechai_auth_status
    },
    {
        "id": "openai-codex",
    ...
    },
    {
        ...
        # as nastechai's device-code flow; the PKCE bit is a security
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            try:
                from nastech_cli.auth import clear_provider_auth, invalidate_nastechai_auth_status_cache
...
#
#   Device code (nastechai, OpenAI Codex):
#     1. POST /api/providers/oauth/{nastechai|openai-codex}/start
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (nastechai, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "nastechai":
        ...
        def _do_nastechai_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_nastechai_device_request
        )
        sid, sess = _new_oauth_session("nastechai", "device_code", profile=profile)
        ...
        threading.Thread(
            target=_nastechai_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to nastechai's device-code
...
def _nastechai_poller(session_id: str) -> None:
    """Background poller that drives a nastechai device-code flow to completion."""
    from nastech_cli.auth import (
        _poll_for_token,
    ...
    try:
        ...
        # Same post-processing as _nastechai_device_code_login (validate/refresh JWT)
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            full_state = refresh_nastechai_oauth_from_state(
                auth_state,
                timeout_seconds=15.0,
                force_refresh=False,
            )
            from nastech_cli.auth import persist_nastechai_credentials
            persist_nastechai_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: nastechai login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("nastechai device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_nastechai_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by nastechai. On success, builds the same
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (nastechai, OpenAI Codex, MiniMax, xAI).
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "nastechai Subscription" image row instead carries the legacy
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "nastechai-blue",     "label": "nastechai Blue",           "description": "Light mode — vivid nastechai-blue accents on cream canvas"},
...
def start_server(
    ...
    try:
        from nastech_cli.nastechai_auth_keepalive import start_nastechai_auth_keepalive

        start_nastechai_auth_keepalive()
    except Exception as exc:
        _log.debug("nastechai auth keepalive did not start: %s", exc)
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            try:
                from plugins.dashboard_auth import nastechai as _nastechai_plugin

                if _nastechai_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • nastechai: {_nastechai_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
