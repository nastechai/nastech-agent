...
from nastech_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _warm_gateway_module() -> None:
    try:
        import nastech_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire nastech_cli.gateway import into a background thread so the event
    # loop is not blocked and NASTECH_DASHBOARD_READY fires without delay.
...
class ModelAssignment(BaseModel):
    ...
    # ``nastech model`` custom flow collects. Honored only on the main slot for
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronastechai*: the whole point of ``debug share`` is the set of shareable
...
def get_recommended_default_model(provider: str = ""):
    ...
    # Non-Nastechai: first curated model for the provider, matching prior behaviour.
    try:
        from nastech_cli.inventory import build_models_payload, load_picker_context
...
def _apply_model_assignment_sync(
    ...
    """Synchronastechai body of POST /api/model/set.
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's NASTECH_HOME.
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        ...
        if provider_id == "nastechai":
            raw = hauth.get_nastechai_auth_status()
            return {
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``nastech mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        ...
        # resolve NASTECH_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = NastechTokenStorage(name)
...
async def list_mcp_catalog(profile: Optional[str] = None):
    """Browse the Nastechai-approved MCP catalog (the optional-mcps/ manifests).
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # No git step — install synchronastechaily via the catalog API. install_entry
        ...
        if cat:
            for prov in _visible_providers(cat, config, force_fresh=True):
                ...
                providers.append({
                    ...
                    "requires_nastechai_auth": bool(prov.get("requires_nastechai_auth")),
                    "is_active": is_active,
                })
    return {
        "name": name,
...
async def _resolve_chat_argv_async(
    ...
    ``_make_tui_argv``.  Keep that synchronastechai work off the WebSocket event
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_nastech_root_is_opt_data() else "local"
...
def start_server(
    ...
    # as agent turns, and a single synchronastechai GIL-holding call on a worker
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"NASTECH_DASHBOARD_READY port={actual_port}", flush=True)
