...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            nastechai_session_valid = get_nous_session_validity()
        except Exception:
            nastechai_session_valid = "unknown"
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nastechai":
            try:
                from nastech_cli.nastechai_subscription import apply_nous_managed_defaults
...
def start_server(
    ...
    try:
        from nastech_cli.nastechai_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            try:
                from plugins.dashboard_auth import nastechai as _nous_plugin

                if _nous_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • nastechai: {_nous_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
