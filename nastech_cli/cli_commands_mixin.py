...
class CLICommandsMixin:
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug nastechai``   → upload to Nastechai-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``nastechai`` and ``local`` are mutually exclusive; if both are given,
        ...
        nastechai = "nastechai" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, nastechai=nastechai, yes=True
        )
