...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      ...
      2. The ``portal_base_url`` stored on the Nastechai login — this is the
    ...
    try:
        from nastech_cli.auth import DEFAULT_NASTECHAI_PORTAL_URL, get_provider_auth_state
...
def _print_post_register_hint(
    *,
    ...
    print()
    print(
        "  Heads up — Nastechai login only *engages* on a non-loopback bind. A plain\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require Nastechai login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require Nastechai login (e.g. exposing on your LAN or a public host):")
...
def cmd_dashboard_register(args) -> None:
    ...
    # 1. Resolve a fresh Nastechai access token (refreshes if near expiry). Fail fast
    ...
    default_portal = "https://portal.nastechairesearch.com"
