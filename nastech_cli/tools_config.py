...
from nastech_cli.nastechai_account import format_nous_portal_entitlement_message
...
TOOL_CATEGORIES = {
    ...
    "browser": {
        ...
        #     billed via NasTechai subscription (requires_nous_auth +
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/nastech-agent:latest"
            )
            return
...
#:
#: Not gated on a NasTechai sign-in here: the six ``bfl_flux3_*`` tools carry
#: ``check_fn=check_bfl_requirements``, so an enabled toolset still ships zero
#: schemas to a user with no NasTechai credential — the same split Home Assistant
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[NasTechaiSubscriptionFeatures] = None,
) -> bool:
...
def _visible_providers(
    ...
    *,
    ...
    NasTechai-managed Tool Gateway rows (``managed_nous_feature``) are always
...
def _agent_browser_installed() -> bool:
    ...
    from nastech_cli.nastechai_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from nastech_cli.nastechai_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    zero-env-var row, including logged-out NasTechai Subscription rows).

    ``features`` (a ``NasTechaiSubscriptionFeatures``) can be passed to avoid
