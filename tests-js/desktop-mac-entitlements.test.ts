...
for (const p of [MAIN_PLIST, INHERIT_PLIST]) {
  test(`${path.basename(p)} is a well-formed non-empty entitlement dict`, () => {
    const data = loadEntitlements(p)
    assert.ok(
      Object.keys(data).length > 0,
      `${path.basename(p)} should be a non-empty dict`
    )
  })
}
